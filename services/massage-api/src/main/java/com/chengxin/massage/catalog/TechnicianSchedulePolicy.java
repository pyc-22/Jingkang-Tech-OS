...
public class TechnicianSchedulePolicy {
  ...
  public void requireClockInEligibility(UUID storeId, UUID technicianId) {
    ...
    if (!result.eligible()) throw new ResponseStatusException(HttpStatus.CONFLICT, clockInMessage(result.reason()));
  }
  ...
  private TechnicianEligibility eligibility(UUID storeId, UUID technicianId, LocalDate date, boolean enabled) {
    ...
    if (technician == null || !Boolean.TRUE.equals(technician.active())) {
      return new TechnicianEligibility(technicianId, false, "INACTIVE");
    }
    if (!Boolean.TRUE.equals(technician.queueEnabled())) {
      return new TechnicianEligibility(technicianId, false, "QUEUE_DISABLED");
    }
    ...
    if (approvedLeave) return new TechnicianEligibility(technicianId, false, "APPROVED_LEAVE");
    ...
    if ("SCHEDULED".equals(status)) {
      ...
      if (shift.startTime() != null && now.isBefore(shift.startTime())) return new TechnicianEligibility(technicianId, false, "NOT_STARTED");
      if (shift.endTime() != null && !now.isBefore(shift.endTime())) {
        ...
        return new TechnicianEligibility(technicianId, serving, serving ? "SERVICE_OVERTIME" : "SHIFT_ENDED");
      }
      return new TechnicianEligibility(technicianId, true, "SCHEDULED");
    }
    if ("REST".equals(status)) return new TechnicianEligibility(technicianId, false, "REST");
    ...
    if ("CANCELLED".equals(status) || status == null) return new TechnicianEligibility(technicianId, true, "AVAILABLE");
    return new TechnicianEligibility(technicianId, true, "AVAILABLE");
  }
  ...
  public record TechnicianEligibility(UUID technicianId, Boolean eligible, String reason) {}
}
