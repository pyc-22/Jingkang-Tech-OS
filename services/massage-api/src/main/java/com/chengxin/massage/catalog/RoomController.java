...
public class RoomController {
  ...
  List<RoomStatus> statuses(@RequestHeader(value = HttpHeaders.AUTHORIZATION, required = false) String authorization, @RequestHeader(value = "X-Store-Id", required = false) String requestedStoreId) {
    UUID storeId = storeContext.currentStore(authorization, requestedStoreId);
    return jdbc.sql("select distinct on (room_id) room_id,status,reason,occurred_at from room_status_event where store_id=:store order by room_id,occurred_at desc")
  ...
  void changeStatus(@PathVariable UUID roomId, @Valid @RequestBody StatusInput input, @RequestHeader(value = HttpHeaders.AUTHORIZATION, required = false) String authorization, @RequestHeader(value = "X-Store-Id", required = false) String requestedStoreId) {
    ...
    boolean hasActiveService = jdbc.sql("select exists(select 1 from service_session where store_id=:store and room_id=:room and status in ('PENDING_ACCEPTANCE','ACCEPTED','REASSIGNMENT_REQUIRED','IN_SERVICE'))")
    ...
    jdbc.sql("insert into room_status_event(id,tenant_id,store_id,room_id,status,reason,source) values(:id,:tenant,:store,:room,:status,:reason,'FRONTDESK')")
  ...
  void completeCleaning(@PathVariable UUID roomId,
    ...
    UUID storeId = storeContext.currentStore(authorization, requestedStoreId);
    Room room = room(storeId, roomId);
    if (!room.active()) throw new ResponseStatusException(HttpStatus.CONFLICT, "房间已停用");
    String status = jdbc.sql("select status from room_status_event where store_id=:store and room_id=:room order by occurred_at desc limit 1")
      .param("store", storeId).param("room", roomId).query(String.class).optional().orElse("IDLE");
    if (!"CLEANING".equals(status)) throw new ResponseStatusException(HttpStatus.CONFLICT, "仅清洁中的房间可以完成清洁");
    boolean hasActiveService = jdbc.sql("select exists(select 1 from service_session where store_id=:store and room_id=:room and status='IN_SERVICE')")
      .param("store", storeId).param("room", roomId).query(Boolean.class).single();
    if (hasActiveService) throw new ResponseStatusException(HttpStatus.CONFLICT, "房间存在进行中的服务，无法完成清洁");
    jdbc.sql("insert into room_status_event(id,tenant_id,store_id,room_id,status,reason,source) values(:id,:tenant,:store,:room,'IDLE','Cleaning completed','FRONTDESK')")
      .param("id", UUID.randomUUID()).param("tenant", TENANT_ID).param("store", storeId).param("room", roomId).update();
    audits.record(authorization, storeId, "ROOM", "ROOM_CLEANING_COMPLETED", "room", roomId, "完成房间清洁", new StatusAudit(status, null), new StatusAudit("IDLE", "Cleaning completed"));
  }
  ...
  void confirmPayment(@PathVariable UUID roomId,
    ...
    String status = jdbc.sql("select status from room_status_event where store_id=:store and room_id=:room order by occurred_at desc limit 1")
      .param("store", storeId).param("room", roomId).query(String.class).optional().orElse("IDLE");
    if (!"PENDING_PAYMENT".equals(status)) throw new ResponseStatusException(HttpStatus.CONFLICT, "仅待付款房间可以确认已付款");
    boolean hasActiveService = jdbc.sql("select exists(select 1 from service_session where store_id=:store and room_id=:room and status='IN_SERVICE')")
      .param("store", storeId).param("room", roomId).query(Boolean.class).single();
    if (hasActiveService) throw new ResponseStatusException(HttpStatus.CONFLICT, "房间仍有进行中的服务，无法确认付款");
    jdbc.sql("insert into room_status_event(id,tenant_id,store_id,room_id,status,reason,source) values(:id,:tenant,:store,:room,'CLEANING','Frontdesk payment confirmed; awaiting cleaning','FRONTDESK_PAYMENT_CONFIRM')")
      .param("id", UUID.randomUUID()).param("tenant", TENANT_ID).param("store", storeId).param("room", roomId).update();
  ...
  private StatusAudit latestStatus(UUID storeId, UUID roomId) {
    return jdbc.sql("select status,reason from room_status_event where store_id=:store and room_id=:room order by occurred_at desc limit 1")
