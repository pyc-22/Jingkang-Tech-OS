...
async function openManagerExtension(sessionId,technicianId){
  ...
  try{
    ...
    const remaining=Math.max(0,Number(policy.serviceDurationMaxMinutes||0)-Number(session.plannedDurationMinutes||0));
    ...
    if(remaining<=0)return managerToast('本次服务已达到门店总时长上限');
    if(!services.length)return managerToast('门店当前没有符合总时长上限的加钟项目');
    ...
    document.querySelector('#manager-extension-summary').textContent=`${servingTechnician.technicianName||session.activeTechnicianName||session.technicianName||'技师'} · ${session.roomCode||'—'} 房 · 当前总时长 ${Number(session.plannedDurationMinutes||0)} 分钟 · 门店总时长剩余 ${remaining} 分钟`;
    document.querySelector('#manager-extension-service').innerHTML=services.map(item=>`<option value="${managerEscape(item.id)}">${managerEscape(item.name)} · ${Number(item.defaultDurationMinutes||0)} 分钟 · ${managerMoney(item.priceCents)}</option>`).join('');
...
function renderManagerClockDialog(){
  ...
  document.querySelector('#manager-clock-tech-list').innerHTML=techs.map(item=>`<button class="manager-clock-tech-choice ${managerClockingTechIds.some(id=>String(id)===String(item.id))?'selected':''}" data-manager-clock-tech="${managerEscape(item.id)}" type="button"><span class="manager-clock-tech-avatar">${managerEscape(String(item.name||'').slice(0,1))}</span><span><b>${managerEscape(item.name)}</b><small>轮钟 ${managerEscape(String(managerQueuePositions.get(String(item.id))||item.queueOrder||'—').padStart(2,'0'))}</small></span><em>${managerTechnicianLiveStatus(item.id)==='IDLE'?'可派':'可预定'}</em></button>`).join('')||'<p class="comparison-empty">当前没有符合条件的技师</p>';
...
function managerLiveTechnicianCard(technician){
  ...
  return `<article class="manager-live-technician ${managerEscape(status)}"><div class="manager-live-technician-top"><div><strong>${managerEscape(technician.technicianCode)} · ${managerEscape(technician.technicianName)}</strong><small>${queue?managerEscape(`当日轮钟第 ${queue} 位`):'暂未进入当日轮钟'}</small></div><div class="manager-live-technician-actions"><span class="manager-technician-status ${managerEscape(status)}">${managerEscape(managerTechnicianStatusLabel[status]||status)}</span>${arrangeButton}</div></div><p>${location}</p><small>${service}${clock}</small><small>${timing}</small></article>`;
}
...
function renderManagerStoreAlerts(rows){document.querySelector('#manager-alert-subtitle').textContent=rows.length?`${rows.length} 项需要处理`:'当前没有待处理事项';document.querySelector('#manager-store-alerts').innerHTML=rows.map(row=>`<article><span class="manager-alert-type ${managerEscape(row.alertType)}">${managerEscape(managerAlertLabel[row.alertType]||row.alertTitle)}</span><div><b>${managerEscape(row.storeName)}</b><small>${managerEscape(row.storeCode)} · ${managerEscape(row.alertCount)} 项</small></div><button type="button" data-manager-alert-store="${managerEscape(row.storeId)}">查看</button></article>`).join('')||'<p class="comparison-empty">当前没有需要处理的经营待办</p>';}
...
function renderManagerCrossStoreTransactions(rows){document.querySelector('#manager-cross-store-records').innerHTML=rows.map(row=>{const amount=Number(row.amountCents||0);const signed=row.transactionType==='CONSUMPTION'?`-${managerMoney(Math.abs(amount))}`:managerMoney(amount);return `<article><span class="manager-cross-store-type ${managerEscape(row.transactionType)}">${managerEscape(managerCrossStoreLabel[row.transactionType]||row.transactionType)}</span><div><b>${managerEscape(row.memberName)} · ${signed}</b><small>${managerEscape(row.storeName)} · ${managerEscape(row.referenceNo)} · ${managerEscape(row.paymentMethod||row.status||'—')}</small><small>${managerEscape(row.serviceTrace||'无关联服务')}</small></div><button type="button" data-manager-cross-store="${managerEscape(row.storeId)}">切换</button></article>`;}).join('')||'<p class="comparison-empty">没有符合条件的记录</p>';}
...
function renderManagerExpenseAttachments(attachments=[]){const target=document.querySelector('#manager-expense-attachments');if(!target)return;target.innerHTML=attachments.map(item=>`<article><span title="${managerExpenseEscape(item.originalFilename)}">${managerExpenseEscape(item.originalFilename)} · ${(Number(item.fileSizeBytes||0)/1024).toFixed(0)} KB</span><span><button type="button" data-expense-preview="${managerEscape(item.id)}" data-expense-preview-name="${managerExpenseEscape(item.originalFilename)}" data-expense-preview-type="${managerExpenseEscape(item.contentType)}">预览</button><button class="danger" type="button" data-expense-attachment="${managerEscape(item.id)}">删除</button></span></article>`).join('');}
