(() => {
  ...
  document.querySelector('#management-view').insertAdjacentHTML('beforebegin', `
    <section class="view daily-report-view hidden" id="daily-report-view">
      ...
      <section class="panel daily-report-refunds hidden" id="daily-report-refunds"><div class="daily-report-section-heading"><div><p class="eyebrow">退款发生明细</p><h2>当日已完成退款</h2></div><span>按实际完成营业日归集</span></div><div id="daily-report-refund-list" class="daily-report-refund-list"></div></section>
  ...
  function render(reportData) {
    ...
    document.querySelector('#daily-report-overview').innerHTML = [
      ['门店名称', selectedStoreName(), '营业日报'], ['营业日期', dateInput.value, '当日数据'], ['当日营业额', cents(values.dailySalesCents), `目标 ${percent(derived.dailyTargetCompletionRate)}`], ['本日红冲退款', cents(reportData.refundAmountCents), '按完成营业日归集'], ['累计营业额', cents(monthly.salesAmountCents), `本月目标 ${percent(derived.monthlyTargetCompletionRate)}`]
