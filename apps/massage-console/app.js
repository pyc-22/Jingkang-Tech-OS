...
async function openDispatchReassignment(sessionId) {
  ...
  document.querySelector('#dispatch-reassignment-slot').innerHTML = unresolved.map(item => `<option value="${item.slotNo}">${item.technicianCode ? `工号 ${dispatchEscape(item.technicianCode)}` : '未设置工号'} · ${dispatchEscape(item.technicianName)} · ${item.status === 'REJECTED' ? '已拒单' : '接单超时'} · 技师位 ${item.slotNo}</option>`).join('');
  document.querySelector('#dispatch-reassignment-technician').innerHTML = candidates.map(item => `<option value="${item.id}">${item.code ? `工号 ${dispatchEscape(item.code)}` : '未设置工号'} · ${dispatchEscape(item.name)} · 轮钟 ${String(item.queue).padStart(2, '0')}${item.state === 'reassign' ? ' · 可重新安排' : ''}</option>`).join('') || '<option value="">当前没有可安排技师</option>';
...
async function openParticipantTransfer(sessionId) {
  ...
  form.reset();
  document.querySelector('#participant-transfer-from').innerHTML = active.map(item => `<option value="${item.technicianId}">${item.technicianCode ? `工号 ${roomTransferEscape(item.technicianCode)}` : '未设置工号'} · ${roomTransferEscape(item.technicianName)} · ${Number(item.allocationBp) / 100}%</option>`).join('');
  document.querySelector('#participant-transfer-to').innerHTML = replacements.map(item => `<option value="${item.id}">${item.code ? `工号 ${roomTransferEscape(item.code)}` : '未设置工号'} · ${roomTransferEscape(item.name)} · 轮钟 ${String(item.queue).padStart(2,'0')}</option>`).join('');
...
function renderTechnicians() {
  ...
  document.querySelector('#technician-list').innerHTML = sorted.map(tech => {
    ...
    return `<article class="technician tech-card state-${tech.state} ${unavailable ? 'unavailable' : ''}" data-tech-card="${tech.id}"><div class="tech-card-main"><span class="tech-avatar">${roomTransferEscape(tech.initials)}</span><span class="technician-name"><b>${tech.code ? `工号 ${roomTransferEscape(tech.code)}` : '未设置工号'}</b><span class="technician-full-name">${roomTransferEscape(tech.name)}</span><small>${room ? `房间 ${roomTransferEscape(room)}` : roomTransferEscape(tech.detail)}</small></span><span class="tech-state ${tech.state}">${label}</span></div><div class="tech-card-meta"><span>排钟 <b>${tech.queueCount}</b> · 点钟 <b>${tech.callCount}</b> · 加钟 <b>${tech.extensionCount}</b></span><span>轮排 ${String(tech.queue).padStart(2, '0')}</span></div>${next}<button class="tech-action" data-tech="${tech.id}"${unavailable ? ' disabled title="当前不可上钟"' : ''}>${action}</button></article>`;
  }).join('');
...
function openClockDialog(tech = null, selectedRoom = null) {
  ...
  document.querySelector('#dispatch-tech-list').innerHTML = availableTechs.map(item => `<button class="dispatch-tech-choice" data-dispatch-tech="${item.id}" type="button"><span class="tech-avatar">${roomTransferEscape(item.initials)}</span><span><b>${item.code ? `工号 ${roomTransferEscape(item.code)}` : '未设置工号'}</b><span class="technician-full-name">${roomTransferEscape(item.name)}</span><small>轮钟 ${String(item.queue).padStart(2, '0')}</small></span><em>${item.state === 'available' ? '可派' : '服务中'}</em></button>`).join('');
