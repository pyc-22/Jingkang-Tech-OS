...
async function openDispatchReassignment(sessionId) {
  ...
  document.querySelector('#dispatch-reassignment-technician').innerHTML = candidates.map(item => `<option value="${item.id}">${dispatchEscape(item.name)} · 工号 ${dispatchEscape(item.code || '未设置')} · 轮钟 ${String(item.queue).padStart(2, '0')}${item.state === 'reassign' ? ' · 可重新安排' : ''}</option>`).join('') || '<option value="">当前没有可安排技师</option>';
...
async function openParticipantTransfer(sessionId) {
  ...
  document.querySelector('#participant-transfer-to').innerHTML = replacements.map(item => `<option value="${item.id}">${roomTransferEscape(item.name)} · 工号 ${roomTransferEscape(item.code || '未设置')} · 轮钟 ${String(item.queue).padStart(2,'0')}</option>`).join('');
...
function renderTechnicians() {
  ...
  document.querySelector('#technician-list').innerHTML = sorted.map(tech => {
    ...
    return `<article class="technician tech-card state-${tech.state} ${unavailable ? 'unavailable' : ''}" data-tech-card="${tech.id}"><div class="tech-card-main"><span class="queue-number">${roomTransferEscape(tech.code || '—')}</span><span class="tech-avatar">${roomTransferEscape(tech.initials)}</span><span class="technician-name"><b>${roomTransferEscape(tech.name)}</b><small>工号 ${roomTransferEscape(tech.code || '未设置')}</small><small>${room ? `房间 ${roomTransferEscape(room)}` : roomTransferEscape(tech.detail)}</small></span><span class="tech-state ${tech.state}">${label}</span></div><div class="tech-card-meta"><span>排钟 <b>${tech.queueCount}</b> · 点钟 <b>${tech.callCount}</b> · 加钟 <b>${tech.extensionCount}</b></span><span>轮排 ${String(tech.queue).padStart(2, '0')}</span></div>${next}<button class="tech-action" data-tech="${tech.id}"${unavailable ? ' disabled title="当前不可上钟"' : ''}>${action}</button></article>`;
  }).join('');
...
function renderHistoricalBackfillPayments() {
  ...
  target.innerHTML = historicalBackfillPayments.map(payment => {
    ...
    return `<div class="historical-backfill-payment" data-historical-payment="${payment.id}"><label class="historical-backfill-payment-method"><span>支付方式</span><select data-historical-payment-method aria-label="支付方式">${methods}</select></label><label class="historical-backfill-payment-amount"><span>金额（元）</span><span class="historical-backfill-currency-input"><i>¥</i><input data-historical-payment-amount type="number" min="0" step="0.01" value="${(payment.amountCents / 100).toFixed(2)}" aria-label="支付金额"></span></label><button class="icon-button historical-backfill-payment-remove" type="button" data-historical-remove-payment="${payment.id}" title="移除支付方式" aria-label="移除支付方式" ${historicalBackfillPayments.length === 1 ? 'disabled' : ''}>×</button></div>`;
  }).join('');
...
function renderHistoricalBackfillManagers(){const target=document.querySelector('#backfill-manager-records');if(!target)return;target.innerHTML=backfillManagers.map(row=>`<tr><td>${historicalBackfillEscape(row.managerLoginName||'—')}</td><td><b>${historicalBackfillEscape(row.managerName||'—')}</b></td><td><span class="record-type ${row.isActive?'order':'consumption'}">${row.isActive?'已授权':'未授权'}</span></td><td>${row.isActive&&row.grantedAt?new Date(row.grantedAt).toLocaleString('zh-CN'):'—'}</td><td>${historicalBackfillEscape(row.grantedByName||'—')}</td><td class="align-right"><button class="record-delete" type="button" data-backfill-manager-toggle="${row.managerId}" data-active="${row.isActive?'false':'true'}">${row.isActive?'撤销授权':'授予授权'}</button></td></tr>`).join('')||'<tr><td colspan="6" class="table-empty">当前门店暂无店长账号</td></tr>';}
...
function syncHistoricalBackfillAccessStores(){const options=accessStores.filter(store=>store.active).map(store=>`<option value="${store.id}">${historicalBackfillEscape(store.code)} · ${historicalBackfillEscape(store.name)}</option>`).join('');for(const selector of ['#backfill-access-store','#historical-backfill-filter-store']){const element=document.querySelector(selector);if(!element)continue;const previous=element.value;element.innerHTML=(selector.endsWith('filter-store')?'<option value="">全部门店</option>':'')+options;if([...element.options].some(option=>option.value===previous))element.value=previous;}if(!historicalBackfillStoreId()&&accessStores.some(store=>store.active))document.querySelector('#backfill-access-store').value=accessStores.find(store=>store.active).id;}
...
function renderDispatchSelection() {
  ...
  document.querySelector('#clock-tech-name').textContent = selectedTechs.length ? selectedTechs.map(tech => `${tech.name}（工号 ${tech.code || '未设置'}）`).join('、') : '未选择';
  ...
  allocationList.innerHTML = selectedTechs.length ? `<div class="dispatch-allocation-heading"><b>技师独立服务配置</b><small>每位技师可单独选择项目、钟类和时长</small></div>${selectedTechs.map(tech => {
    ...
    return `<fieldset class="dispatch-participant ${String(tech.id) === String(dispatchFocusTechId) ? 'focused' : ''}" data-dispatch-participant="${dispatchEscape(tech.id)}"><legend>${dispatchEscape(tech.name)} · 工号 ${dispatchEscape(tech.code || '未设置')}</legend><div class="dispatch-participant-grid"><label>项目<select data-tech-service="${dispatchEscape(tech.id)}">${options}</select></label><label>钟类<select data-tech-clock-type="${dispatchEscape(tech.id)}"><option value="QUEUE" ${choice.clockType === 'QUEUE' ? 'selected' : ''}>排钟</option><option value="CALL" ${choice.clockType === 'CALL' ? 'selected' : ''}>点钟</option><option value="SELECTED" ${choice.clockType === 'SELECTED' ? 'selected' : ''}>选钟</option><option value="BOOKED_QUEUE" ${choice.clockType === 'BOOKED_QUEUE' ? 'selected' : ''}>预定排钟</option><option value="BOOKED_CALL" ${choice.clockType === 'BOOKED_CALL' ? 'selected' : ''}>预定点钟</option></select></label><label>时长<input data-tech-duration="${dispatchEscape(tech.id)}" type="number" min="15" max="360" step="5" value="${Number(choice.durationMinutes || 60)}"><small>分钟</small></label></div></fieldset>`;
...
function openClockDialog(tech = null, selectedRoom = null) {
  ...
  document.querySelector('#dispatch-tech-list').innerHTML = availableTechs.map(item => `<button class="dispatch-tech-choice" data-dispatch-tech="${item.id}" type="button"><span class="tech-avatar">${roomTransferEscape(item.initials)}</span><span><b>${roomTransferEscape(item.name)}</b><small>工号 ${roomTransferEscape(item.code || '未设置')} · 轮钟 ${String(item.queue).padStart(2, '0')}</small></span><em>${item.state === 'available' ? '可派' : '服务中'}</em></button>`).join('');
