...
function renderTechnicians() {
  ...
  document.querySelector('#technician-list').innerHTML = sorted.map(tech => {
    ...
    return `<article class="technician tech-card state-${roomTransferEscape(tech.state)} ${unavailable ? 'unavailable' : ''}" data-tech-card="${roomTransferEscape(tech.id)}"><div class="tech-card-main"><span class="tech-avatar" style="font-size:${Math.min(18, Math.floor(36 / Math.ceil(Math.sqrt(String(tech.code || '').length || 1))))}px">${tech.code ? code : '-'}</span><span class="technician-name"><b>${code}</b><span class="technician-full-name">${roomTransferEscape(tech.name)}</span><small>${room ? `房间 ${roomTransferEscape(room)}` : roomTransferEscape(tech.detail)}</small></span><span class="tech-state ${roomTransferEscape(tech.state)}">${label}</span></div><div class="tech-card-meta"><span>排钟 <b>${roomTransferEscape(tech.queueCount)}</b> · 点钟 <b>${roomTransferEscape(tech.callCount)}</b> · 加钟 <b>${roomTransferEscape(tech.extensionCount)}</b></span><span>轮排 ${roomTransferEscape(String(tech.queue).padStart(2, '0'))}</span></div>${next}<button class="tech-action" data-tech="${roomTransferEscape(tech.id)}"${unavailable ? ' disabled title="当前不可上钟"' : ''}>${action}</button></article>`;
  }).join('');
...
function openClockDialog(tech = null, selectedRoom = null) {
  ...
  document.querySelector('#dispatch-tech-list').innerHTML = availableTechs.map(item => { const code = item.code ? roomTransferEscape(item.code) : '未设置工号'; return `<button class="dispatch-tech-choice" data-dispatch-tech="${roomTransferEscape(item.id)}" type="button"><span class="tech-avatar" style="font-size:${Math.min(18, Math.floor(36 / Math.ceil(Math.sqrt(String(item.code || '').length || 1))))}px">${item.code ? code : '-'}</span><span><b>${code}</b><span class="technician-full-name">${roomTransferEscape(item.name)}</span><small>轮钟 ${roomTransferEscape(String(item.queue).padStart(2, '0'))}</small></span><em>${item.state === 'available' ? '可派' : '服务中'}</em></button>`; }).join('');
