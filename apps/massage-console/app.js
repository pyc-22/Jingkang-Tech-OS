...
function openTechnicianRoomTransfer(sessionId) {
  ...
  const session = [...state.activeSessions, ...(state.pendingAcceptanceSessions || []), ...(state.acceptedSessions || [])]
    .find(item => String(item.id) === String(sessionId));
  if (!session) return toast('未找到进行中的服务记录，请刷新后重试');
  const rooms = state.rooms.filter(room => Number(room.availableBedCount || 0) > 0 && ['idle', 'reserved', 'serving'].includes(room.status) && String(room.apiId) !== String(session.roomId));
  if (!rooms.length) return toast('当前没有可用床位可供换房');
  ...
  form.reset();
   document.querySelector('#technician-room-transfer-summary').textContent = `${sessionDisplayTechnicianName(session)} · ${session.serviceNameSnapshot} · 当前 ${session.roomCode} 房`;
    document.querySelector('#technician-room-transfer-target').innerHTML = rooms.map(room => `<option value="${roomTransferEscape(room.apiId)}">${roomTransferEscape(room.id)} 房 · ${roomTransferEscape(room.availableBedCount)}/${roomTransferEscape(room.bedCount)} 床可用</option>`).join('');
...
function renderDispatchSelection() {
  ...
  document.querySelector('#clock-tech-name').textContent = selectedTechs.length ? selectedTechs.map(tech => `${tech.code || '未设置工号'} · ${tech.name}`).join('、') : '未选择';
  ...
  allocationList.innerHTML = selectedTechs.length ? `<div class="dispatch-allocation-heading"><b>技师独立服务配置</b><small>每位技师可单独选择项目、钟类和时长</small></div>${selectedTechs.map(tech => {
    ...
    return `<fieldset class="dispatch-participant ${String(tech.id) === String(dispatchFocusTechId) ? 'focused' : ''}" data-dispatch-participant="${dispatchEscape(tech.id)}"><legend>${tech.code ? dispatchEscape(tech.code) : '未设置工号'} · ${dispatchEscape(tech.name)}</legend><div class="dispatch-participant-grid"><label>项目<select data-tech-service="${dispatchEscape(tech.id)}">${options}</select></label><label>钟类<select data-tech-clock-type="${dispatchEscape(tech.id)}"><option value="QUEUE" ${choice.clockType === 'QUEUE' ? 'selected' : ''}>排钟</option><option value="CALL" ${choice.clockType === 'CALL' ? 'selected' : ''}>点钟</option><option value="SELECTED" ${choice.clockType === 'SELECTED' ? 'selected' : ''}>选钟</option><option value="BOOKED_QUEUE" ${choice.clockType === 'BOOKED_QUEUE' ? 'selected' : ''}>预定排钟</option><option value="BOOKED_CALL" ${choice.clockType === 'BOOKED_CALL' ? 'selected' : ''}>预定点钟</option></select></label><label>时长<input data-tech-duration="${dispatchEscape(tech.id)}" type="number" min="15" max="360" step="5" value="${Number(choice.durationMinutes || 60)}"><small>分钟</small></label></div></fieldset>`;
...
function openClockDialog(tech = null, selectedRoom = null) {
  ...
  document.querySelector('#dispatch-tech-list').innerHTML = availableTechs.map(item => { const code = item.code ? roomTransferEscape(item.code) : '未设置工号'; return `<button class="dispatch-tech-choice" data-dispatch-tech="${roomTransferEscape(item.id)}" type="button"><span class="tech-avatar" title="${code}" style="font-size:${Math.min(18, Math.floor(36 / Math.ceil(Math.sqrt(String(item.code || '').length || 1))))}px">${item.code ? code : '-'}</span><span class="technician-name"><span class="technician-full-name">${roomTransferEscape(item.name)}</span>${!item.code ? '<small>未设置工号</small>' : ''}</span><em class="dispatch-tech-status">${item.state === 'available' ? '可派钟' : '服务中'}</em><small class="dispatch-tech-meta">轮钟 ${roomTransferEscape(String(item.queue).padStart(2, '0'))}</small></button>`; }).join('');
...
function financeClaimQuery() {
  ...
  // Empty UUID values must be omitted for Spring's typed binding.
  for(const key of ['storeId','categoryId'])if(!query.get(key))query.delete(key);
  query.set('sort',financeClaimSort);query.set('direction',financeClaimDirection);return query;
}
function updateFinanceSelection() {
  const eligible=financeClaims.filter(row=>row.status==='SUBMITTED');
  const all=document.querySelector('#finance-select-all');
  all.checked=eligible.length>0&&eligible.every(row=>financeSelected.has(row.id));
  all.indeterminate=financeSelected.size>0&&!all.checked;all.disabled=!eligible.length;
  document.querySelector('#finance-selected-count').textContent=`已选 ${financeSelected.size} 单`;
  for(const id of ['finance-batch-approve','finance-batch-reject'])document.getElementById(id).disabled=!financeSelected.size;
}
function openFinanceBatch(action) {
  if(!financeSelected.size)return;
  financeBatchAction=action;
  financeBatchIds=[...financeSelected];
  document.querySelector('#finance-batch-title').textContent=action==='APPROVED'?'批量通过':'批量驳回';
  document.querySelector('#finance-batch-summary').textContent=`已选 ${financeSelected.size} 条待审核报销`;
  document.querySelector('#finance-batch-form').reset();document.querySelector('#finance-batch-dialog').showModal();
}
async function submitFinanceBatch(event) {
  event.preventDefault();const form=event.currentTarget,comment=form.elements.comment.value.trim();
  if(!comment)return toast('请填写统一审核意见');
  const submit=form.querySelector('[type="submit"]');if(submit.disabled)return;submit.disabled=true;
  try {
    const response=await fetch('http://localhost:8080/api/v1/finance/expense-claims/batch-review',{method:'POST',headers:adminJsonHeaders(),body:JSON.stringify({ids:financeBatchIds,action:financeBatchAction,comment})});
    if(!response.ok)throw new Error(await financeError(response));
    financeSelected.clear();document.querySelector('#finance-batch-dialog').close();await loadFinanceClaims(0);toast('批量审核已完成');
  }catch(error){toast(error.message||'批量审核失败');}finally{submit.disabled=false;}
}
function setupFinanceClaimBrowser() {
  const reload=()=>loadFinanceClaims().catch(error=>toast(error.message||'报销加载失败'));
  document.querySelector('#finance-report-metrics').addEventListener('click',event=>{
    const button=event.target.closest('[data-report-status]');if(!button)return;
    for(const key of ['from','to'])document.querySelector('#finance-claims-'+key).value=document.querySelector('#finance-report-'+key).value;
    document.querySelector('#finance-store-filter').value=document.querySelector('#finance-report-store').value;
    document.querySelector('#finance-status-filter').value=button.dataset.reportStatus;
    for(const id of ['finance-applicant-filter','finance-category-filter','finance-claims-number'])document.getElementById(id).value='';
    reload();document.querySelector('.finance-metrics').scrollIntoView({block:'start',behavior:'smooth'});
  });
  document.querySelector('#finance-category-filter').addEventListener('change',reload);
  let timer;document.querySelector('#finance-applicant-filter').addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(reload,300);});
  document.querySelectorAll('[data-finance-status]').forEach(button=>button.addEventListener('click',()=>{const select=document.querySelector('#finance-status-filter');select.value=select.value===button.dataset.financeStatus?'':button.dataset.financeStatus;reload();}));
  document.querySelectorAll('[data-expense-preset]').forEach(button=>button.addEventListener('click',()=>{const range=ExpenseUI.range(button.dataset.expensePreset);for(const key of ['from','to'])document.querySelector('#finance-claims-'+key).value=range[key];reload();}));
  document.querySelector('#finance-claims-reset').addEventListener('click',()=>{
    document.querySelectorAll('.finance-claim-filters input,.finance-claim-filters select').forEach(input=>input.value='');financeClaimSort='time';financeClaimDirection='desc';updateFinanceSort();reload();
  });
  document.querySelectorAll('[data-finance-sort]').forEach(button=>button.addEventListener('click',()=>{const sort=button.dataset.financeSort;financeClaimDirection=sort===financeClaimSort&&financeClaimDirection==='desc'?'asc':'desc';financeClaimSort=sort;updateFinanceSort();reload();}));
  document.querySelector('#finance-page-prev').addEventListener('click',()=>loadFinanceClaims(financeClaimPage-1).catch(error=>toast(error.message)));
  document.querySelector('#finance-page-next').addEventListener('click',()=>loadFinanceClaims(financeClaimPage+1).catch(error=>toast(error.message)));
  document.querySelector('#finance-claim-records').addEventListener('change',event=>{const id=event.target.dataset.financeSelect;if(!id)return;event.target.checked?financeSelected.add(id):financeSelected.delete(id);updateFinanceSelection();});
  document.querySelector('#finance-select-all').addEventListener('change',event=>{financeSelected=event.target.checked?new Set(financeClaims.filter(row=>row.status==='SUBMITTED').map(row=>row.id)):new Set();renderFinanceClaims(financeClaims);});
  document.querySelector('#finance-batch-approve').addEventListener('click',()=>openFinanceBatch('APPROVED'));
  document.querySelector('#finance-batch-reject').addEventListener('click',()=>openFinanceBatch('REJECTED'));
  document.querySelector('#finance-batch-close').addEventListener('click',()=>document.querySelector('#finance-batch-dialog').close());
  document.querySelector('#finance-batch-form').addEventListener('submit',submitFinanceBatch);
  document.querySelector('#finance-claims-export').addEventListener('click',async event=>{
    const button=event.currentTarget;button.disabled=true;
    try{await ExpenseUI.download(`http://localhost:8080/api/v1/finance/expense-claims/export?${financeClaimQuery()}`,adminHeaders());}catch(error){toast(error.message);}finally{button.disabled=false;}
  });
  for(const [id,delta] of [['finance-attachment-prev',-1],['finance-attachment-next',1]])document.getElementById(id).addEventListener('click',()=>{
    const file=financeActiveClaim?.attachments[financeAttachmentIndex+delta];if(file)openFinanceAttachment(file.id,file.originalFilename,file.contentType).catch(error=>toast(error.message));
  });
  document.querySelector('#finance-attachment-dialog').addEventListener('close',()=>{financeAttachmentRequest++;if(financeAttachmentObjectUrl){URL.revokeObjectURL(financeAttachmentObjectUrl);financeAttachmentObjectUrl=null;}});
}
function updateFinanceSort() {
  document.querySelectorAll('[data-sort-column]').forEach(th=>{const active=th.dataset.sortColumn===financeClaimSort;th.setAttribute('aria-sort',active?(financeClaimDirection==='asc'?'ascending':'descending'):'none');th.querySelector('span').textContent=active?(financeClaimDirection==='asc'?'↑':'↓'):'↕';});
}
setupFinanceClaimBrowser();
...
function renderFinanceClaims(rows) {
  const target=document.querySelector('#finance-claim-records');
...
function financeReportSetDefaults(){const range=ExpenseUI.range();for(const key of ['from','to']){const input=document.querySelector('#finance-report-'+key);if(input&&!input.value)input.value=range[key];}}
...
function renderFinanceDetail(detail,targetId) {
  const claim=detail.claim||{}, attachments=detail.attachments||[], history=detail.history||[], payment=detail.payment;
  const field=(label,value)=>`<span>${label}<strong>${financeEscape(value??'未记录')}</strong></span>`;
  const block=(title,body)=>`<section class="finance-detail-block"><h3>${title}</h3>${body}</section>`;
  const latestReview=history.find(item=>['APPROVE','RETURN','REJECT'].includes(item.action));
  ...
  document.querySelector(targetId).innerHTML=
    ...
    block('操作历史',`<ol class="finance-detail-history">${history.map(item=>`<li><b>${financeDetailAction[item.action]||financeEscape(item.action)} · ${financeDetailStatus[item.toStatus]||financeEscape(item.toStatus)}</b><small>${financeEscape(ExpenseUI.time(item.createdAt))} · ${financeEscape(item.actorName||'系统')}</small><p>${financeEscape(item.comment||'')}</p></li>`).join('')||'<li>暂无操作记录</li>'}</ol>`);
  const actions=document.querySelector('#finance-review-actions');if(actions)actions.hidden=claim.status!=='SUBMITTED';
}
async function openFinanceAttachment(id,filename,contentType) {
  if(!financeActiveClaim)return;
  const request=++financeAttachmentRequest,claimId=financeActiveClaim.claim.id;
  const response=await fetch(`http://localhost:8080/api/v1/finance/expense-claims/${claimId}/attachments/${id}`,{headers:adminHeaders()});
  if(!response.ok)return toast(`附件加载失败：${await financeError(response)}`);
  const blob=await response.blob();
  if(request!==financeAttachmentRequest||financeActiveClaim.claim.id!==claimId)return;
  if(financeAttachmentObjectUrl)URL.revokeObjectURL(financeAttachmentObjectUrl);
  financeAttachmentObjectUrl=URL.createObjectURL(blob);
  document.querySelector('#finance-attachment-title').textContent=filename||'报销附件';
  const target=document.querySelector('#finance-attachment-preview');
  if((contentType||blob.type).startsWith('image/'))target.innerHTML=`<img src="${financeAttachmentObjectUrl}" alt="${financeEscape(filename||'报销附件')}" />`;
  else if((contentType||blob.type)==='application/pdf')target.innerHTML=`<iframe src="${financeAttachmentObjectUrl}" title="${financeEscape(filename||'报销附件')}"></iframe>`;
  else target.textContent='当前文件类型暂不支持在线预览，请下载后查看。';
  financeAttachmentIndex=financeActiveClaim.attachments.findIndex(item=>item.id===id);
  document.querySelector('#finance-attachment-position').textContent=`${financeAttachmentIndex+1} / ${financeActiveClaim.attachments.length}`;
  document.querySelector('#finance-attachment-prev').disabled=financeAttachmentIndex<=0;
  document.querySelector('#finance-attachment-next').disabled=financeAttachmentIndex>=financeActiveClaim.attachments.length-1;
  if(!document.querySelector('#finance-attachment-dialog').open)document.querySelector('#finance-attachment-dialog').showModal();
}
...
async function loadFinanceClaims(page=null) {
  ...
  const result=await response.json();
  if(request!==financeClaimRequest)return;
  if(key!==financeClaimQueryKey)financeSelected.clear();
  financeClaimQueryKey=key;financeClaimPage=requestedPage;financeClaims=result.items;
  financeSelected=new Set([...financeSelected].filter(id=>financeClaims.some(row=>row.id===id&&row.status==='SUBMITTED')));
  const select=document.querySelector('#finance-store-filter'),current=select.value;
  select.innerHTML='<option value="">全部门店</option>'+result.stores.map(store=>`<option value="${financeEscape(store.id)}">${financeEscape(store.name)}</option>`).join('');select.value=current;
  const summary=result.summary;
  document.querySelector('#finance-record-count').textContent=summary.claimCount;
  document.querySelector('#finance-total-amount').textContent=financeAmount(summary.effectiveAmountCents);
  document.querySelector('#finance-pending-count').textContent=summary.pendingCount;
  document.querySelector('#finance-approved-count').textContent=summary.approvedCount;
  document.querySelector('#finance-paid-count').textContent=summary.paidCount;
  document.querySelector('#finance-approved-amount').textContent=financeAmount(summary.approvedAmountCents);
  document.querySelector('#finance-paid-amount').textContent=financeAmount(summary.paidAmountCents);
  document.querySelectorAll('[data-finance-status]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.financeStatus===query.get('status'))));
  renderFinanceClaims(financeClaims);
  document.querySelector('#finance-page-count').textContent=`本页 ${result.items.length} 条 / 共 ${result.total} 条`;
  document.querySelector('#finance-page-number').textContent=`${requestedPage+1} / ${Math.max(1,Math.ceil(result.total/20))}`;
  document.querySelector('#finance-page-prev').disabled=requestedPage===0;
  document.querySelector('#finance-page-next').disabled=(requestedPage+1)*20>=result.total;
  document.querySelector('#finance-sync-status').textContent=ExpenseUI.time(new Date())+' 已同步';
...
async function loadMemberSalesStats(){setupMemberSalesStats();const date=document.querySelector('#member-sales-date').value||localDateValue();const cards=document.querySelector('#member-sales-cards');const tech=document.querySelector('#member-sales-tech');cards.classList.add('is-loading');try{const txResponse=await fetch(`http://localhost:8080/api/v1/wallet-transactions?from=${date}&to=${date}`,{headers:storeContextHeaders()});if(!txResponse.ok)throw new Error('SALES_STATS_LOAD_FAILED');const rows=await txResponse.json();let openCount=0,openAmount=0,renewAmount=0,consumption=0;const technicians=new Map();for(const row of rows){if(row.reportExcluded)continue;const amount=Math.abs(Number(row.rechargeAmountCents??row.amountCents??0));if(row.transactionType==='RECHARGE'){if(row.openingRecharge){openCount++;openAmount+=amount;}else renewAmount+=amount;const name=row.technicianNameSnapshot||'未关联技师';technicians.set(name,(technicians.get(name)||0)+amount);}else if(row.transactionType==='CONSUMPTION')consumption+=amount;}document.querySelector('#member-sales-cards').innerHTML=`<article><span>开卡数量</span><strong>${openCount}</strong></article><article><span>售卡金额</span><strong>${money(openAmount/100)}</strong></article><article><span>续卡金额</span><strong>${money(renewAmount/100)}</strong></article><article><span>卡耗金额</span><strong>${money(consumption/100)}</strong></article>`;const ranking=[...technicians.entries()].sort((a,b)=>b[1]-a[1]);tech.innerHTML=`<h4>充值技师归属</h4>${ranking.map(([name,amount])=>`<div><span>${memberBusinessEscape(name)}</span><strong>${money(amount/100)}</strong></div>`).join('')||'<p>该日期暂无充值归属记录</p>'}`;}catch{cards.innerHTML='<p class="member-sales-error">售卡统计加载失败，请稍后重试</p>';tech.innerHTML='';}finally{cards.classList.remove('is-loading');}}
