...
function serviceCategoryOptions(selectedId = '', excludedId = '') {
  ...
  const render = (parentId = 'ROOT', depth = 0) => (children.get(parentId) || []).map(item => {
    ...
    return `<option value="${item.id}"${selected}>${'　'.repeat(depth)}${roomTransferEscape(item.name)}</option>${render(String(item.id), depth + 1)}`;
  }).join('');
  return render();
}
...
function groupedServiceSelectOptions(services) {
  ...
  return [...groups.entries()].sort(([left], [right]) => left.localeCompare(right, 'zh-CN')).map(([path, items]) => `<optgroup label="${roomTransferEscape(path)}">${items.sort((left, right) => left.name.localeCompare(right.name, 'zh-CN')).map(item => `<option value="${item.id}">${roomTransferEscape(item.name)} · ${roomTransferEscape(item.duration)} · ¥${Number(item.price || 0).toFixed(2)}</option>`).join('')}</optgroup>`).join('');
}
function ensureRoomTransferPanel() {
  if (!document.querySelector('#room-transfer-panel')) {
    document.querySelector('#frontdesk-view .frontdesk-layout')?.insertAdjacentHTML('afterend', '<section class="panel room-transfer-panel" id="room-transfer-panel"><div class="panel-heading"><div><h2>待处理换房</h2><p>技师提交后由前台确认，原房间确认后进入清洁</p></div><button class="text-button" id="refresh-room-transfers" type="button">刷新</button></div><div class="room-transfer-list" id="room-transfer-list"><p class="table-empty">暂无待处理换房申请</p></div></section>');
  }
  if (!document.querySelector('#room-transfer-reject-dialog')) {
    document.body.insertAdjacentHTML('beforeend', '<dialog id="room-transfer-reject-dialog"><form id="room-transfer-reject-form" class="dialog-card compact"><div class="dialog-heading"><div><p class="eyebrow">换房申请</p><h2>拒绝换房</h2></div><button class="icon-button" type="button" id="close-room-transfer-reject" aria-label="关闭">×</button></div><label class="form-full room-transfer-reject-label">拒绝原因<textarea name="note" rows="3" maxlength="240" required placeholder="请填写拒绝原因"></textarea></label><div class="dialog-actions"><button class="button secondary" type="button" id="cancel-room-transfer-reject">取消</button><button class="button primary" type="submit">确认拒绝</button></div></form></dialog>');
    document.querySelector('#close-room-transfer-reject').addEventListener('click', () => document.querySelector('#room-transfer-reject-dialog').close());
    document.querySelector('#cancel-room-transfer-reject').addEventListener('click', () => document.querySelector('#room-transfer-reject-dialog').close());
    document.querySelector('#room-transfer-reject-form').addEventListener('submit', async event => {
      event.preventDefault();
      const note = new FormData(event.currentTarget).get('note')?.trim();
      if (!activeRoomTransferRejectId || !note) return;
      const response = await fetch(`http://localhost:8080/api/v1/service-room-transfers/${activeRoomTransferRejectId}/reject`, { method:'POST', headers:storeContextHeaders(true), body:JSON.stringify({ note }) });
      if (!response.ok) return toast('拒绝换房失败，请刷新后重试');
      document.querySelector('#room-transfer-reject-dialog').close();
      activeRoomTransferRejectId = null;
      await Promise.all([loadFoundationData({ silent:true }), loadRoomTransferRequests({ silent:true })]);
      toast('换房申请已拒绝，目标房间已恢复空闲');
    });
  }
}
function renderRoomTransferRequests() {
  const target = document.querySelector('#room-transfer-list');
  if (!target) return;
  const rows = state.pendingRoomTransfers || [];
  target.innerHTML = rows.map(item => `<article class="room-transfer-row"><div class="room-transfer-main"><div class="room-transfer-route"><strong>${roomTransferEscape(item.fromRoomCode)}</strong><span>→</span><strong>${roomTransferEscape(item.toRoomCode)}</strong><span class="room-transfer-status requested">待确认</span></div><p>${roomTransferEscape(item.technicianName || '技师')} · ${roomTransferEscape(item.serviceNameSnapshot || '服务项目')}</p><small>${roomTransferEscape(item.reason || '未填写原因')} · ${item.requestedAt ? new Date(item.requestedAt).toLocaleString('zh-CN') : ''}</small></div><div class="room-transfer-actions"><button class="button primary" type="button" data-transfer-approve="${item.id}">确认换房</button><button class="button secondary" type="button" data-transfer-reject="${item.id}">拒绝</button></div></article>`).join('') || '<p class="table-empty">暂无待处理换房申请</p>';
}
...
async function loadFoundationData({ silent = false } = {}) {
  try {
    ...
    state.technicians = technicians.map((tech, index) => { const schedule=eligibilityByTechnician.get(tech.id); const eligible=!schedule||schedule.eligible; return { id: tech.id, code:tech.code, name: tech.name, initials: tech.name.slice(0, 1), queueEnabled: tech.queueEnabled !== false, state: eligible?'available':'off', queue: queueByTechnician.get(String(tech.id)) ?? tech.queueOrder ?? index + 1, detail:eligible?'可立即安排服务':(technicianScheduleReasonLabel[schedule?.reason]||'当前不可上钟'), queueCount:0, callCount:0, extensionCount:0, nextReservation:waitingReservationByTechnician.get(String(tech.id)) || null }; });
    ...
    await loadRoomTransferRequests({ silent: true });
...
async function openDispatchReassignment(sessionId) {
  ...
  document.querySelector('#dispatch-reassignment-technician').innerHTML = candidates.map(item => `<option value="${item.id}">${dispatchEscape(item.name)} · 轮钟 ${String(item.queue).padStart(2, '0')}${item.state === 'reassign' ? ' · 可重新安排' : ''}</option>`).join('') || '<option value="">当前没有可安排技师</option>';
...
async function openParticipantTransfer(sessionId) {
  ...
  form.reset();
  document.querySelector('#participant-transfer-from').innerHTML = active.map(item => `<option value="${item.technicianId}">${item.technicianCode} · ${item.technicianName} · ${Number(item.allocationBp) / 100}%</option>`).join('');
  document.querySelector('#participant-transfer-to').innerHTML = replacements.map(item => `<option value="${item.id}">${item.name} · 轮钟 ${String(item.queue).padStart(2,'0')}</option>`).join('');
...
async function openFrontdeskExtensionCancel(sessionId) {
  ...
  document.querySelector('#frontdesk-extension-cancel-target').innerHTML = extensions.map(item => `<option value="${item.id}">${roomTransferEscape(item.serviceNameSnapshot)} · ${item.plannedDurationMinutes} 分钟 · ¥${(Number(item.servicePriceCents || 0) / 100).toFixed(2)}</option>`).join('');
...
function openTechnicianRoomTransfer(sessionId) {
  ...
  form.reset();
   ...
   document.querySelector('#technician-room-transfer-target').innerHTML = rooms.map(room => `<option value="${room.apiId}">${roomTransferEscape(room.id)} 房</option>`).join('');
...
async function submitTechnicianRoomTransfer(event) {
  ...
  try {
    ...
    if (!approved.ok) { toast('换房申请已创建，请在待处理换房中确认'); await loadRoomTransferRequests({ silent:true }); return; }
...
function renderTechnicians() {
  ...
  document.querySelector('#technician-list').innerHTML = sorted.map(tech => {
    ...
    return `<article class="technician tech-card state-${tech.state} ${unavailable ? 'unavailable' : ''}" data-tech-card="${tech.id}"><div class="tech-card-main"><span class="queue-number">${roomTransferEscape(tech.code || String(tech.queue).padStart(2, '0'))}</span><span class="tech-avatar">${roomTransferEscape(tech.initials)}</span><span class="technician-name"><b>${roomTransferEscape(tech.name)}</b><small>${room ? `房间 ${roomTransferEscape(room)}` : roomTransferEscape(tech.detail)}</small></span><span class="tech-state ${tech.state}">${label}</span></div><div class="tech-card-meta"><span>排钟 <b>${tech.queueCount}</b> · 点钟 <b>${tech.callCount}</b> · 加钟 <b>${tech.extensionCount}</b></span><span>轮排 ${String(tech.queue).padStart(2, '0')}</span></div>${next}<button class="tech-action" data-tech="${tech.id}"${unavailable ? ' disabled title="当前不可上钟"' : ''}>${action}</button></article>`;
  }).join('');
...
function renderManagedTechnicians() {
  ...
  document.querySelector('#technician-records').innerHTML = rows.map(tech => {
    ...
    return `<tr><td>${tech.queueOrder}</td><td>${tech.code}</td><td><b>${tech.name}</b></td><td class="muted-cell">${tech.phone || '—'}</td><td><span class="record-type ${tech.active ? 'order' : 'consumption'}">${tech.active ? '在职' : '已离职'}</span></td><td><span class="record-type ${queueClass}">${tech.active ? queueState : '—'}</span></td><td class="align-right">${tech.active ? `<button class="record-delete edit-technician" data-edit="${tech.id}">编辑</button><button class="record-delete" data-queue-toggle="${tech.id}" data-enabled="${tech.queueEnabled !== false}">${queueAction}</button><button class="record-delete" data-toggle="${tech.id}" data-active="true">删除</button>` : '<span class="muted-cell">—</span>'}</td></tr>`;
...
function renderManagedEmployees() {
  ...
  document.querySelector('#employee-records').innerHTML = rows.map(employee => {
    ...
    const createProfile = employee.active && employee.employmentStatus === 'ACTIVE' && employee.positionType === 'TECHNICIAN' && !employee.technicianId
      ? `<button class="record-delete edit-technician" type="button" data-employee-create-technician="${employee.employeeId}">创建技师档案</button>` : '';
    return `<tr><td>${employee.employeeNo || '—'}</td><td><b>${employee.fullName}</b>${tech}</td><td>${employee.positionName}<small class="muted-cell">${employeePositionLabel[employee.positionType] || employee.positionType}</small></td><td class="muted-cell">${employee.phone || '—'}</td><td>${employee.hiredOn || '—'}</td><td>${employee.loginName || '<span class="muted-cell">未关联</span>'}</td><td><span class="record-type ${statusClass}">${employeeStatusLabel[employee.employmentStatus] || employee.employmentStatus}</span></td><td class="align-right"><button class="record-delete edit-technician" type="button" data-employee-edit="${employee.employeeId}">编辑</button>${createProfile}${employee.employmentStatus === 'ACTIVE' ? `<button class="record-delete" type="button" data-employee-leave="${employee.assignmentId}">登记离职</button>` : ''}</td></tr>`;
...
function renderEmployeeAccounts() {
  ...
  if (!target) return;
  target.innerHTML = managedEmployees.map(employee => `<tr><td><b>${employee.fullName}</b><small class="muted-cell"> ${employee.employeeNo || '未设置员工号'}</small></td><td>${employee.positionName}</td><td>${employee.loginName || '<span class="muted-cell">未关联</span>'}</td><td>${employee.technicianId ? '<span class="record-type order">已关联</span>' : '<span class="muted-cell">普通员工</span>'}</td><td><span class="record-type ${employee.userId ? 'order' : 'consumption'}">${employee.userId ? '已关联' : '未关联'}</span></td><td class="align-right"><button class="record-delete edit-technician" type="button" data-employee-account="${employee.employeeId}">${employee.userId ? '调整账号' : '关联账号'}</button></td></tr>`).join('') || '<tr><td colspan="6" class="table-empty">当前门店没有员工档案</td></tr>';
}
...
async function loadEmployeeAccountOptions(employee) {
  ...
  document.querySelector('#employee-account-select').innerHTML = `<option value="">暂不关联账号</option>${selected.map(item => `<option value="${item.id}">${item.loginName}${item.displayName ? ` · ${item.displayName}` : ''}</option>`).join('')}`;
...
function renderEmployeeAttendance() {
  ...
  target.innerHTML = employeeAttendance.map(item => {
    ...
    const inAction = !item.clockInAt && !['REST','ABSENT','LEAVE'].includes(item.status) ? `<button class="record-delete edit-technician" type="button" data-attendance-clock-in="${item.employeeId}">上班打卡</button>` : '';
    const outAction = item.clockInAt && !item.clockOutAt ? `<button class="record-delete" type="button" data-attendance-clock-out="${item.employeeId}">下班打卡</button>` : '';
    ...
    return `<tr><td><b>${roomTransferEscape(item.employeeName)}</b></td><td>${roomTransferEscape(item.positionName)}<small class="muted-cell">${employeePositionLabel[item.positionType] || item.positionType}</small></td><td>${shift}</td><td>${formatAttendanceTime(item.clockInAt)}</td><td>${formatAttendanceTime(item.clockOutAt)}</td><td>${minutes}</td><td><span class="record-type ${stateClass}">${attendanceStatusLabel[item.status] || item.status}</span></td><td class="align-right">${inAction}${outAction || '<span class="muted-cell">—</span>'}</td></tr>`;
...
async function loadAdministrativeCommissionSummary() {
  ...
  const employeeOptions=employees.map(item=>`<option value="${item.employeeId}">${roomTransferEscape(item.fullName)} · ${roomTransferEscape(item.positionName)}</option>`).join('');
  ...
  const serviceOptions=services.map(item=>`<option value="${item.id}">${roomTransferEscape(item.name)}</option>`).join('');
...
function priceVersionSummary(item) {
  const scheduled = item.scheduledPriceEffectiveBusinessDate
    ? `<span class="version-scheduled">预约 ${item.scheduledPriceEffectiveBusinessDate}：${money(item.scheduledPriceCents / 100)}</span>`
    : '';
  return `<div class="version-cell"><b>${money(item.priceCents / 100)}</b><small>${versionDateLabel(item.priceEffectiveBusinessDate)}生效</small>${scheduled}</div>`;
}
...
function commissionVersionSummary(rule) {
  ...
  const scheduled = rule.scheduledEffectiveBusinessDate
    ? `<small class="version-scheduled">预约 ${rule.scheduledEffectiveBusinessDate}</small>`
    : '';
  return `${commissionRuleSummary(rule)}<small class="version-current">${versionDateLabel(rule.effectiveBusinessDate)}生效</small>${scheduled}`;
}
...
function renderManagedServiceItems() {
  ...
  document.querySelector('#service-item-records').innerHTML = rows.map(item => {
    ...
    return `<tr><td>${item.code}</td><td><b>${item.name}</b></td><td>${roomTransferEscape(item.categoryId ? serviceCategoryPath(item.categoryId) : item.category)}</td><td><span class="record-type ${item.countsAsClock ? 'order' : 'consumption'}">${item.countsAsClock ? '计钟' : '不计钟'}</span></td><td>${item.defaultDurationMinutes} 分钟</td><td class="amount-cell">${priceVersionSummary(item)}</td><td>${item.requiresRoom ? '需要房间' : '不需要房间'}</td><td>${item.allowsExtension ? '可加钟' : '不可加钟'}</td><td>${commissionVersionSummary(rule)}</td><td><span class="record-type ${item.active ? 'order' : 'consumption'}">${item.active ? '启用' : '已删除'}</span></td><td class="align-right">${item.active ? `<button class="record-delete edit-technician" type="button" data-service-item-commission="${item.id}">提成</button><button class="record-delete edit-technician" type="button" data-service-item-edit="${item.id}">编辑</button><button class="record-delete" type="button" data-service-item-active="${item.id}" data-active="true">删除</button>` : '<span class="muted-cell">—</span>'}</td></tr>`;
...
async function loadPriceVersionHistory(item) {
  ...
  target.innerHTML = `<b>价格版本（新版本生效前沿用上一版本）</b><div class="version-history-list">${versions.map(version => `<span class="${version.effectiveBusinessDate === item.scheduledPriceEffectiveBusinessDate ? 'version-scheduled' : version.effectiveBusinessDate === item.priceEffectiveBusinessDate ? 'version-current' : ''}">${versionDateLabel(version.effectiveBusinessDate)} · ${money(version.priceCents / 100)}</span>`).join('')}</div>`;
}
...
async function loadCommissionVersionHistory(item, currentRule) {
  ...
  const versions = await response.json();
  target.innerHTML = `<b>提成版本（新版本生效前沿用上一版本）</b><div class="version-history-list">${versions.map(version => `<span class="${version.effectiveBusinessDate === currentRule?.scheduledEffectiveBusinessDate ? 'version-scheduled' : version.effectiveBusinessDate === currentRule?.effectiveBusinessDate ? 'version-current' : ''}">${versionDateLabel(version.effectiveBusinessDate)} · 排 ${commissionRuleText(version.queueRuleType, version.queueFixedCents, version.queueRateBp)} / 点 ${commissionRuleText(version.callRuleType, version.callFixedCents, version.callRateBp)} / 加 ${commissionRuleText(version.extensionRuleType, version.extensionFixedCents, version.extensionRateBp)}</span>`).join('')}</div>`;
}
...
function renderManagedPaymentMethods() {
  ...
  document.querySelector('#payment-method-records').innerHTML = rows.map(item => `<tr><td>${item.code}</td><td><b>${item.name}</b>${item.builtIn ? '<small class="muted-cell"> 系统内置</small>' : ''}</td><td>${item.cashCounted ? '计入现金统计' : '不计入现金统计'}</td><td class="muted-cell">${item.note || '—'}</td><td><span class="record-type ${item.active ? 'order' : 'consumption'}">${item.active ? '启用' : '已删除'}</span></td><td class="align-right">${item.builtIn || !item.active ? '<span class="muted-cell">—</span>' : `<button class="record-delete edit-technician" type="button" data-payment-method-edit="${item.id}">编辑</button><button class="record-delete" type="button" data-payment-method-active="${item.id}" data-active="true">删除</button>`}</td></tr>`).join('') || '<tr><td colspan="6" class="table-empty">没有匹配的收款方式</td></tr>';
}
...
async function printReceipt(orderDetail, openedWindow = null) {
  ...
  const lineRows = lines.map(line => `<tr><td>${receiptEscape(line.itemNameSnapshot)}${show('showClockType') ? `<small>钟类：${receiptEscape(receiptClockTypeLabel(line.clockType))}</small>` : ''}${show('showTechnician') && line.technicianName ? `<small>技师：${receiptEscape(line.technicianName)}</small>` : ''}${show('showRoom') && line.roomCode ? `<small>房间：${receiptEscape(line.roomCode)}</small>` : ''}</td><td>${line.durationMinutes}分</td><td>¥${(line.lineAmountCents / 100).toFixed(2)}</td></tr>`).join('');
...
async function printReferenceReceipt(orderDetail, openedWindow = null) {
  ...
  const lineRows = lines.map((line, index) => `<tr><td>${index + 1}</td><td>${receiptEscape(line.itemNameSnapshot)}${show('showClockType') ? `<small>钟类：${receiptEscape(receiptClockTypeLabel(line.clockType))}</small>` : ''}</td><td>${show('showTechnician') ? receiptEscape(line.technicianName || '—') : ''}</td><td>${line.quantity || 1}</td><td>${moneyText(line.lineAmountCents)}</td><td>—</td></tr>`).join('');
  const compactLineRows = lines.map((line, index) => `<article class="reference-compact-item"><div><b>${index + 1}. ${receiptEscape(line.itemNameSnapshot)}</b><strong>${moneyText(line.lineAmountCents)}</strong></div>${show('showClockType') ? `<small>钟类：${receiptEscape(receiptClockTypeLabel(line.clockType))}</small>` : ''}${show('showTechnician') ? `<small>技师：${receiptEscape(line.technicianName || '—')}</small>` : ''}<small>${line.quantity || 1} × ${line.durationMinutes || 0}分</small></article>`).join('');
...
async function printBrowserReceiptByOptions(orderDetail, openedWindow = null) {
  ...
  const itemSection = show('showProjects') ? (isNarrow
    ...
    : `<table class="item-table"><colgroup>${itemColumns.map(column => `<col class="${column.className}">`).join('')}</colgroup><thead><tr>${itemColumns.map(column => `<th class="${column.className}">${column.label}</th>`).join('')}</tr></thead><tbody>${itemRows}</tbody></table>`) : '';
...
async function loadManagedRooms() {
  ...
  document.querySelector('#room-records').innerHTML = managedRooms.map(room => `<tr><td>${room.code}</td><td><b>${room.name}</b></td><td>${room.roomType}</td><td>${room.bedCount}</td><td><span class="record-type ${room.active ? 'order' : 'consumption'}">${room.active ? '启用' : '已删除'}</span></td><td class="align-right">${room.active ? `<button class="record-delete edit-technician" data-room-edit="${room.id}">编辑</button><button class="record-delete" data-room-active="${room.id}" data-active="true">删除</button>` : '<span class="muted-cell">—</span>'}</td></tr>`).join('');
}
...
function renderPendingServiceSessions() {
  ...
  const clockType = clockTypeLabels[session.clockType] || session.clockType;
    ...
    return `<div class="pending-service-row"><button class="pending-service-item" type="button" data-pending-service="${session.id}" ${selected ? 'disabled' : ''}><span><b>${memberBusinessEscape(session.serviceNameSnapshot)}</b><small>${memberBusinessEscape(session.serviceNo || '')} · ${clockType} · ${memberBusinessEscape(session.technicianName)} · ${memberBusinessEscape(session.roomCode)} 房 · ${session.plannedDurationMinutes} 分钟${memberBusinessEscape(extension)}</small></span><em>${selected ? '已加入' : money(session.servicePriceCents / 100)}</em></button><button class="pending-service-void" type="button" data-void-service="${session.id}" title="作废未结算服务" aria-label="作废 ${memberBusinessEscape(session.serviceNameSnapshot)}">作废</button></div>`;
...
function openPendingServiceVoid(sessionId) {
  ...
  document.querySelector('#pending-service-void-summary').innerHTML = `<span><b>${memberBusinessEscape(session.roomCode)} 房 · ${memberBusinessEscape(session.serviceNameSnapshot)}</b><small>${memberBusinessEscape(session.technicianName)} · ${session.plannedDurationMinutes} 分钟 · ${session.businessDate || '当前营业日'}</small></span><strong>${money(Number(session.servicePriceCents || 0) / 100)}</strong>`;
...
async function openOrderCorrection(orderId){
  ...
  document.querySelector('#order-correction-summary').innerHTML=`<span><b>${memberBusinessEscape(order.orderNo)}</b><small>${detail.lines.length} 个项目 · ${memberBusinessEscape(order.memberName||'散客')} · 原实收 ${money(Number(order.paidCents||0)/100)}</small></span>`;
...
async function openFinancialCorrection(orderId){
  ...
  document.querySelector('#financial-correction-summary').innerHTML=`<span><b>原实收 ${money(Number(detail.order.paidCents||0)/100)}</b><small>${memberBusinessEscape(detail.order.memberName||'散客')}${detail.order.memberId?` · 当前会员余额 ${money(Number(detail.order.memberBalanceCents||0)/100)}`:''}</small></span>`;
...
function scheduleTechnicianOptions(selected=''){return managedTechnicians.map(technician=>`<option value="${technician.id}" ${technician.id===selected?'selected':''}>${technician.code} · ${technician.name}${technician.active?'':'（已删除）'}</option>`).join('')||'<option value="">当前门店没有技师</option>';}
function renderTechnicianSchedules(){document.querySelector('#technician-schedule-records').innerHTML=technicianSchedules.map(item=>`<tr><td><b>${item.technicianName}</b><small class="muted-cell"> ${item.technicianCode}</small></td><td>${scheduleTypeLabel[item.shiftType]||item.shiftType}</td><td>${item.shiftType==='REST'?'全天休息':`${scheduleTime(item.startTime)} - ${scheduleTime(item.endTime)}`}</td><td><span class="record-type ${item.status==='SCHEDULED'?'order':'consumption'}">${scheduleStatusLabel[item.status]||item.status}</span></td><td class="muted-cell">${item.note||'—'}</td><td class="align-right">${item.status==='SCHEDULED'?`<button class="record-delete edit-technician" data-schedule-edit="${item.id}">编辑</button><button class="record-delete" data-schedule-cancel="${item.id}">取消班次</button>`:'—'}</td></tr>`).join('')||'<tr><td colspan="6" class="table-empty">当前日期尚未安排班次</td></tr>';}
function renderTechnicianLeaves(){document.querySelector('#technician-leave-records').innerHTML=technicianLeaveRequests.map(item=>`<tr><td><b>${item.technicianName}</b><small class="muted-cell"> ${item.technicianCode}</small></td><td>${item.startDate}${item.endDate===item.startDate?'':` 至 ${item.endDate}`}</td><td class="muted-cell">${item.reason||'—'}</td><td><span class="record-type ${item.status==='APPROVED'?'consumption':item.status==='PENDING'?'refund-state':'order'}">${leaveStatusLabel[item.status]||item.status}</span></td><td class="muted-cell">${item.reviewedByName?`${item.reviewedByName}${item.reviewNote?` · ${item.reviewNote}`:''}`:'—'}</td><td class="align-right">${item.status==='PENDING'?`<button class="record-delete edit-technician" data-leave-review="${item.id}">审批</button>`:'—'}</td></tr>`).join('')||'<tr><td colspan="6" class="table-empty">当前日期没有相关请假记录</td></tr>';}
...
function renderStoreComparison(rows){document.querySelector('#store-comparison-records').innerHTML=rows.map((row,index)=>`<tr><td><span class="performance-rank ${index<3?'top':''}">${index+1}</span></td><td><b>${row.storeName}</b><small class="muted-cell"> ${row.storeCode}</small></td><td class="amount-cell">${money(row.salesAmountCents/100)}</td><td class="amount-cell">${money(row.serviceAmountCents/100)}<small class="muted-cell"> ${row.completedServiceCount} 次</small></td><td class="amount-cell">${money(row.rechargeAmountCents/100)}</td><td class="amount-cell">${money(row.refundAmountCents/100)}</td><td class="amount-cell">${money(row.cashNetCents/100)}</td><td>服务中 ${row.roomServingCount}/${row.activeRoomCount}<small class="muted-cell">清洁 ${row.roomCleaningCount} · 技师 ${row.activeTechnicianCount}</small></td><td>待结算 ${row.pendingSettlementCount}<small class="muted-cell">待退款 ${row.pendingRefundCount}</small></td></tr>`).join('')||'<tr><td colspan="9" class="table-empty">当前账号未分配可对比门店</td></tr>';}
...
function renderStoreAlerts(rows){document.querySelector('#store-alert-total').textContent=`${rows.length} 项待处理`;document.querySelector('#store-alert-records').innerHTML=rows.map((row,index)=>`<tr><td><span class="store-alert-priority p${Math.min(index,4)}">${index+1}</span></td><td><b>${row.storeName}</b><small class="muted-cell"> ${row.storeCode}</small></td><td><span class="record-type ${row.alertType==='PENDING_REFUND'?'refund-state':'consumption'}">${storeAlertLabel[row.alertType]||row.alertTitle}</span><small class="muted-cell"> ${row.alertTitle}</small></td><td class="amount-cell">${row.alertCount} 项</td><td><button class="record-delete edit-technician" data-store-alert-store="${row.storeId}" data-store-alert-type="${row.alertType}">${storeAlertAction[row.alertType]||'查看'}</button></td></tr>`).join('')||'<tr><td colspan="5" class="table-empty">当前没有需要处理的经营待办</td></tr>';}
...
function renderCrossStoreTransactions(rows){document.querySelector('#cross-store-transaction-records').innerHTML=rows.map(row=>{const amount=Number(row.amountCents||0);const signed=row.transactionType==='CONSUMPTION'||row.transactionType==='REFUND'?signedMoneyCents(-Math.abs(amount)):signedMoneyCents(amount);const service=row.serviceTrace||'—';return `<tr><td><span class="record-type ${row.transactionType==='REFUND'?'refund-state':row.transactionType==='ORDER'?'order':'consumption'}">${crossStoreTransactionLabel[row.transactionType]||row.transactionType}</span></td><td><b>${row.storeName}</b><small class="muted-cell"> ${row.storeCode}</small></td><td><b>${row.referenceNo}</b></td><td>${row.memberName}<small class="muted-cell"> ${row.memberPhone||''}</small></td><td class="amount-cell">${signed}</td><td>${row.paymentMethod||'—'}<small class="muted-cell"> ${row.status||''}</small></td><td class="cross-store-service">${service}</td><td>${row.occurredAt?new Date(row.occurredAt).toLocaleString('zh-CN'):'—'}</td><td><button class="record-delete edit-technician" data-cross-store-transaction-store="${row.storeId}" data-cross-store-transaction-type="${row.transactionType}" data-cross-store-transaction-order="${row.orderId||''}" data-cross-store-transaction-member="${row.memberPhone||''}">${row.orderId?'查看明细':'查看账本'}</button></td></tr>`;}).join('')||'<tr><td colspan="9" class="table-empty">没有符合条件的跨门店记录</td></tr>';}
...
function renderStoreSelector(){
  ...
  container.innerHTML=`<select id="current-store-select" aria-label="当前门店">${activeStores.map(store=>`<option value="${store.id}" ${store.id===selected?'selected':''}>${store.name}</option>`).join('')}</select>`;
...
function renderHeadquartersOverview(stores,rows,alerts,date){const activeStores=stores.filter(store=>store.active);const total=field=>rows.reduce((sum,row)=>sum+Number(row[field]||0),0);const stats=[['启用门店',`${activeStores.length}`,`共 ${stores.length} 家已建档门店`],['净营业额',headquartersMoney(total('salesAmountCents')),`${total('completedServiceCount')} 次完成服务`],['会员充值',headquartersMoney(total('rechargeAmountCents')),`退款 ${headquartersMoney(total('refundAmountCents'))}`],['待处理事项',`${alerts.length}`,`清洁、退款与待结算提醒`]];document.querySelector('#headquarters-overview-date').textContent=`经营日期 ${date}`;document.querySelector('#headquarters-overview-stats').innerHTML=stats.map(item=>`<article><span>${item[0]}</span><strong>${item[1]}</strong><small>${item[2]}</small></article>`).join('');document.querySelector('#headquarters-overview-records').innerHTML=rows.map(row=>`<tr><td><b>${row.storeName}</b><small class="muted-cell">${row.storeCode}</small></td><td class="amount-cell">${headquartersMoney(row.salesAmountCents)}</td><td class="amount-cell">${headquartersMoney(row.serviceAmountCents)}</td><td class="amount-cell">${headquartersMoney(row.rechargeAmountCents)}</td><td class="amount-cell">${headquartersMoney(row.refundAmountCents)}</td><td class="amount-cell">${headquartersMoney(row.cashNetCents)}</td><td>服务中 ${row.roomServingCount}/${row.activeRoomCount}<small class="muted-cell">清洁 ${row.roomCleaningCount}</small></td><td>${row.activeTechnicianCount}</td><td class="align-right"><button class="record-delete edit-technician" type="button" data-headquarters-store="${row.storeId}">进入门店</button></td></tr>`).join('')||'<tr><td colspan="9" class="table-empty">当前没有可汇总的启用门店</td></tr>';}
...
function memberRechargeAttribution(row){
  return `<div class="member-recharge-cell"><b>${row.lastRechargeTechnicianName||'未关联技师'}</b><small>${row.lastRechargeEmployeeName?`经办：${row.lastRechargeEmployeeName}`:'未关联经办员工'}</small></div>`;
}
...
function renderMemberCenter(rows,{replace=true}={}){
  ...
  document.querySelector('#member-center-records').innerHTML=visible.map(row=>`<tr><td><b>${row.code}</b></td><td><div class="member-identity"><b>${row.name}</b><small>${row.phone}</small></div></td><td><div class="member-recharge-cell"><b>${row.registeredStoreName||'—'}</b><small>开卡 ${memberCenterTime(row.createdAt)}</small></div></td><td class="amount-cell align-right">${memberCenterAmount(row.balanceCents)}</td><td class="amount-cell align-right">${memberCenterAmount(row.rechargeCents)}</td><td class="amount-cell align-right">${memberCenterAmount(row.bonusCents)}</td><td class="amount-cell align-right">${memberCenterAmount(row.consumptionCents)}</td><td><div class="member-recharge-cell"><b>${row.lastActivityAt?walletTypeLabel[row.lastActivityType]||row.lastActivityType:'暂无业务'}</b><small>${memberCenterTime(row.lastActivityAt)}${row.lastConsumptionAt?` · 最近消费 ${memberCenterTime(row.lastConsumptionAt)}`:''}</small></div></td><td>${memberRechargeAttribution(row)}</td><td class="align-right"><button class="record-delete edit-technician" type="button" data-member-profile="${row.id}">详情</button><button class="record-delete" type="button" data-member-recharge="${row.id}">充值</button></td></tr>`).join('')||'<tr><td colspan="10" class="table-empty">没有匹配的会员</td></tr>';
}
...
function renderMemberProfile(profile){
  ...
  document.querySelector('#member-profile-overview').innerHTML=`<div><b>${member.name}</b><small>${member.code} · ${member.phone}</small><small>开卡门店：${member.registeredStoreName||'—'}</small></div><div><small>当前余额</small><strong>${memberCenterAmount(member.balanceCents)}</strong></div><div><small>本店累计充值</small><strong>${memberCenterAmount(member.rechargeCents)}</strong><small>赠送 ${memberCenterAmount(member.bonusCents)}</small></div><div><small>本店累计消费</small><strong>${memberCenterAmount(member.consumptionCents)}</strong></div>`;
  const consumptions=profile.transactions.filter(row=>row.transactionType==='CONSUMPTION');
  document.querySelector('#member-profile-transactions').innerHTML=consumptions.map(row=>`<tr><td>${memberCenterTime(row.createdAt)}</td><td><div class="member-recharge-cell"><b>${row.serviceItems||'会员消费'}</b><small>${row.serviceEndedAt?`服务结束 ${memberCenterTime(row.serviceEndedAt)}`:'订单消费'}</small></div></td><td>${row.serviceTechnicianNames||row.technicianNameSnapshot||'未关联技师'}</td><td>${row.roomNames||'—'}</td><td class="wallet-debit align-right">-${memberCenterAmount(Math.abs(row.amountCents))}</td><td class="amount-cell align-right">${memberCenterAmount(row.balanceAfterCents)}</td><td>${row.orderNo||'—'}</td></tr>`).join('')||'<tr><td colspan="7" class="table-empty">当前门店暂无消费明细</td></tr>';
  document.querySelector('#member-profile-wallet-transactions').innerHTML=profile.transactions.map(row=>{const positive=Number(row.amountCents)>=0;const method=row.paymentMethodNameSnapshot||paymentMethodLabel[row.paymentMethod]||row.paymentMethod||'—';return `<tr><td>${memberCenterTime(row.createdAt)}</td><td><span class="record-type ${positive?'order':'consumption'}">${walletTypeLabel[row.transactionType]||row.transactionType}</span></td><td class="${positive?'wallet-credit':'wallet-debit'} align-right">${positive?'+':'-'}${memberCenterAmount(Math.abs(row.amountCents))}</td><td>${method}</td><td class="muted-cell">${memberWalletAttribution(row)}</td></tr>`;}).join('')||'<tr><td colspan="5" class="table-empty">当前门店暂无资金流水</td></tr>';
}
...
async function loadMemberRechargeOptions(){
  ...
  form.technicianId.innerHTML=`<option value="">未关联技师</option>${technicians.map(item=>`<option value="${item.id}">${roomTransferEscape(item.code||'')} · ${roomTransferEscape(item.name)}</option>`).join('')}`;
  form.employeeId.innerHTML=`<option value="">未关联员工</option>${employees.map(item=>`<option value="${item.employeeId}">${roomTransferEscape(item.employeeNo||'')} · ${roomTransferEscape(item.fullName)}${item.positionName?` · ${roomTransferEscape(item.positionName)}`:''}</option>`).join('')}`;
  form.paymentMethod.innerHTML=methods.map(item=>`<option value="${item.code}">${item.name}</option>`).join('');
...
async function loadSalesOrders({resetPage=false}={}){
  ...
  document.querySelector('#order-records').innerHTML=salesOrders.map(o=>{const statusLabel=o.status==='CANCELLED'?'已作废':(refundStatusLabel[o.refundStatus]||'已结算');const canVoid=o.status!=='CANCELLED'&&Number(o.paidCents||0)===0&&o.refundStatus==='NONE';return `<tr><td>${memberBusinessEscape(o.orderNo)}</td><td>${memberBusinessEscape(o.settlementNo||'—')}</td><td>${memberBusinessEscape(o.memberName||'散客')}<small class="muted-cell"> ${memberBusinessEscape(o.memberPhone||'')}</small></td><td class="amount-cell">${money(o.receivableCents/100)}</td><td class="amount-cell">${money(o.paidCents/100)}</td><td>${o.settledAt?new Date(o.settledAt).toLocaleString('zh-CN'):''}</td><td><span class="record-type ${o.status==='CANCELLED'?'refund-state':o.refundStatus==='NONE'?'order':'refund-state'}">${statusLabel}</span></td><td><button class="record-delete edit-technician" type="button" data-order-detail="${o.id}">查看/打印</button>${canVoid?`<button class="record-delete" type="button" data-order-void="${o.id}">作废</button>`:''}</td></tr>`;}).join('')||'<tr><td colspan="8" class="table-empty">暂无订单</td></tr>';
...
async function loadRefundManagement(){const response=await fetch(`http://localhost:8080/api/v1/refunds?status=${refundManagementStatus}`,{headers:storeContextHeaders()});if(!response.ok){document.querySelector('#refund-management-records').innerHTML='<tr><td colspan="9" class="table-empty">退款与红冲记录加载失败</td></tr>';return;}const rows=await response.json();document.querySelector('#refund-management-records').innerHTML=rows.map(row=>{const refund=row.refund;const pending=row.payments.filter(payment=>payment.status==='PENDING');const hasCompleted=row.payments.some(payment=>payment.status==='COMPLETED');const paymentText=row.payments.map(payment=>`${paymentMethodLabel[payment.paymentMethod]||payment.paymentMethod}${payment.status==='PENDING'?'（待确认退款）':''}`).join('、');const actors=`${refund.requestedByNameSnapshot||'—'}<small class="muted-cell">${refund.completedByNameSnapshot?`完成：${refund.completedByNameSnapshot}`:'尚未完成'}</small>`;return `<tr><td><b>${refund.refundNo}</b><small class="muted-cell">${refund.reason}</small></td><td><span class="record-type ${refund.refundKind==='FULL_REVERSAL'?'refund-state':'consumption'}">${refundKindLabel[refund.refundKind]||refund.refundKind}</span></td><td><button class="record-delete edit-technician" data-order-detail="${refund.orderId}">${refund.orderNo}</button></td><td>${refund.memberName||'散客'}<small class="muted-cell">${refund.memberPhone||''}</small></td><td>${paymentText}</td><td class="amount-cell refund-signed-amount">${signedMoneyCents(refund.signedTotalCents||-refund.totalCents)}</td><td>${actors}<small class="muted-cell">${new Date(refund.createdAt).toLocaleString('zh-CN')}</small></td><td><span class="record-type ${refund.status==='COMPLETED'?'order':refund.status==='PENDING'?'refund-state':'consumption'}">${refund.status==='PENDING'?'待确认退款':refund.status==='COMPLETED'?'已完成':'已取消'}</span></td><td class="align-right">${pending.map(payment=>`<button class="record-delete edit-technician" data-dashboard-confirm="${refund.id}" data-dashboard-payment="${payment.id}">确认${paymentMethodLabel[payment.paymentMethod]||payment.paymentMethod}</button>`).join('')}${refund.status==='PENDING'&&!hasCompleted?`<button class="record-delete" data-dashboard-cancel="${refund.id}">取消</button>`:''}</td></tr>`;}).join('')||'<tr><td colspan="9" class="table-empty">暂无符合条件的退款与红冲记录</td></tr>';}
async function loadMemberWalletLedger(){const query=document.querySelector('#wallet-ledger-search').value.trim();const from=document.querySelector('#wallet-ledger-from').value;const to=document.querySelector('#wallet-ledger-to').value;const response=await fetch(`http://localhost:8080/api/v1/wallet-transactions?query=${encodeURIComponent(query)}&from=${from}&to=${to}`,{headers:storeContextHeaders()});if(!response.ok){document.querySelector('#wallet-ledger-records').innerHTML='<tr><td colspan="8" class="table-empty">资金账本加载失败</td></tr>';return;}const rows=await response.json();document.querySelector('#wallet-ledger-records').innerHTML=rows.map(row=>{const positive=Number(row.amountCents)>=0;const signed=`${positive?'+':'-'}${money(Math.abs(row.amountCents)/100)}`;const refundAction=row.transactionType==='RECHARGE'&&hasAdminPermission('ORDER_REFUND')?`<button class="record-delete" type="button" data-recharge-refund="${row.id}" data-recharge-member="${row.memberId}" data-recharge-name="${memberBusinessEscape(row.memberName)}" data-recharge-amount="${row.amountCents}">申请退款</button>`:'';return `<tr><td>${new Date(row.createdAt).toLocaleString('zh-CN')}</td><td><b>${row.memberName}</b><small class="muted-cell"> ${row.memberPhone||row.memberCode}</small></td><td><span class="record-type ${positive?'order':'consumption'}">${walletTypeLabel[row.transactionType]||row.transactionType}</span></td><td class="wallet-amount ${positive?'credit':'debit'}">${signed}</td><td>${money(row.balanceBeforeCents/100)}</td><td class="amount-cell">${money(row.balanceAfterCents/100)}</td><td>${row.source||'—'}</td><td class="muted-cell">${row.note||'—'} ${refundAction}</td></tr>`;}).join('')||'<tr><td colspan="8" class="table-empty">暂无符合条件的资金流水</td></tr>';}
...
async function loadTechnicianPerformance(){const from=document.querySelector('#performance-from').value;const to=document.querySelector('#performance-to').value;const response=await fetch(`http://localhost:8080/api/v1/technician-performance?from=${from}&to=${to}`,{headers:storeContextHeaders()});if(!response.ok){document.querySelector('#technician-performance-records').innerHTML='<tr><td colspan="7" class="table-empty">技师业绩加载失败</td></tr>';return;}const rows=await response.json();document.querySelector('#technician-performance-records').innerHTML=rows.map((row,index)=>`<tr><td><span class="performance-rank ${index<3?'top':''}">${index+1}</span></td><td><b>${row.technicianName}</b><small class="muted-cell"> ${row.technicianCode}</small></td><td class="performance-projects">${row.projectNames||'—'}</td><td>${row.completedCount} 次</td><td>${row.totalMinutes} 分钟</td><td>${money(row.averageAmountCents/100)}</td><td class="amount-cell">${money(row.amountCents/100)}</td></tr>`).join('')||'<tr><td colspan="7" class="table-empty">所选日期没有已完成服务</td></tr>';}
...
function renderManagementOverview(data) {
  ...
  document.querySelector('#management-attention').innerHTML=attention.map(item=>`<div><span class="warning-dot"></span><p><b>${attentionLabel[item.attentionType]||roomTransferEscape(item.title)}</b><small>${Number(item.count||0)} 项需要处理</small></p><button class="text-button" type="button" data-management-attention="${item.attentionType}">查看</button></div>`).join('')||'<div class="management-overview-empty">当前没有待处理事项</div>';
}
...
const accessChecks=(items,selected,key,label)=>items.map(item=>`<label><input type="checkbox" value="${item.id}" ${selected.includes(item.id)?'checked':''}>${item[label]||item[key]}</label>`).join('');
...
function renderAccess(){document.querySelector('#access-store-records').innerHTML=accessStores.map(store=>`<tr><td>${store.code}</td><td><b>${store.name}</b><small class="muted-cell"> ${store.address||''}</small></td><td>${store.timezone}</td><td>${(store.businessDayCutoff||'05:00').slice(0,5)}</td><td>${store.contactPhone||'—'}</td><td>${store.businessHours||'—'}</td><td><span class="record-type ${store.active?'order':'consumption'}">${store.active?'启用':'已删除'}</span></td><td class="align-right">${store.active?`<button class="record-delete edit-technician" data-access-store-edit="${store.id}">编辑</button><button class="record-delete" data-access-store-active="${store.id}" data-active="true">删除</button>`:'<span class="muted-cell">—</span>'}</td></tr>`).join('')||'<tr><td colspan="8" class="table-empty">暂无门店</td></tr>';document.querySelector('#access-user-records').innerHTML=accessUsers.map(user=>`<tr><td>${user.loginName}</td><td><b>${user.displayName}</b></td><td>${roleNames(user.roleIds)}</td><td>${storeNames(user.storeIds)}</td><td><span class="record-type ${user.active?'order':'consumption'}">${user.active?'启用':'已删除'}</span></td><td class="align-right">${user.active?`<button class="record-delete edit-technician" data-access-user-edit="${user.id}">权限</button><button class="record-delete" data-access-user-active="${user.id}" data-active="true">删除</button>`:'<span class="muted-cell">—</span>'}</td></tr>`).join('')||'<tr><td colspan="6" class="table-empty">暂无管理账号</td></tr>';document.querySelector('#access-role-select').innerHTML=accessRoles.map(role=>`<option value="${role.id}">${role.name} · ${role.code}</option>`).join('');renderRolePermissions();}
function renderRolePermissions(){const role=accessRoles.find(item=>item.id===document.querySelector('#access-role-select').value)||accessRoles[0];if(!role)return;document.querySelector('#access-permission-list').innerHTML=accessPermissions.map(permission=>`<label><input type="checkbox" value="${permission.id}" ${role.permissionIds.includes(permission.id)?'checked':''}><span><b>${permission.name}</b><small>${permission.module} · ${permission.code}</small></span></label>`).join('');}
async function loadAccessControl(){if(!isTenantAdmin())return;const [stores,roles,permissions,users]=await Promise.all(['stores','roles','permissions','users'].map(path=>fetch(`http://localhost:8080/api/v1/admin/access/${path}`,{headers:adminHeaders()}).then(response=>response.ok?response.json():Promise.reject())));accessStores=stores;accessRoles=roles;accessPermissions=permissions;accessUsers=users;renderAccess();await loadMyStores();}
...
function renderRefundHistory(refunds){if(!refunds.length)return'<p class="empty-state refund-empty">暂无退款或红冲记录</p>';return `<section class="refund-history"><h3>退款与红冲记录</h3>${refunds.map(item=>{if(!item)return'';const r=item.refund;const pending=item.payments.filter(payment=>payment.status==='PENDING');const canCancel=r.status==='PENDING'&&!item.payments.some(payment=>payment.status==='COMPLETED');const signed=Number(r.signedTotalCents||-r.totalCents);const actor=`申请：${r.requestedByNameSnapshot||'—'}${r.completedByNameSnapshot?` · 完成：${r.completedByNameSnapshot}`:''}`;return `<div class="refund-history-row"><span><b>${refundKindLabel[r.refundKind]||r.refundKind} · ${r.refundNo}</b><small>${r.reason} · ${actor} · ${new Date(r.createdAt).toLocaleString('zh-CN')}</small></span><em class="refund-signed-amount">${signedMoneyCents(signed)} · ${r.status==='PENDING'?'待确认退款':r.status==='COMPLETED'?'已完成':'已取消'}</em>${pending.map(payment=>`<button class="record-delete edit-technician" data-confirm-refund="${r.id}" data-refund-payment="${payment.id}">确认${paymentMethodLabel[payment.paymentMethod]||payment.paymentMethod}退款</button>`).join('')}${canCancel?`<button class="record-delete" data-cancel-refund="${r.id}">取消申请</button>`:''}</div>`;}).join('')}</section>`;}
...
openOrderDetail=async function(orderId){
  ...
  if(actions&&canVoid&&hasAdminPermission('ORDER_REFUND'))actions.insertAdjacentHTML('beforeend',`<button class="button danger" type="button" data-order-void="${order.id}">作废订单</button>`);
  if(actions&&hasAdminPermission('ORDER_REFUND')){
    const terminal=order.status==='CANCELLED'||order.refundStatus==='FULL';
    const label=terminal?'重新结算':Number(order.paidCents||0)>0?'红冲后修正':'修改订单';
    actions.insertAdjacentHTML('beforeend',`<button class="button primary" type="button" data-order-correct="${order.id}">${label}</button>`);
    const financialAllowed=order.status==='SETTLED'&&order.refundStatus==='NONE'&&!activeOrderDetail.refunds.length;
    actions.insertAdjacentHTML('beforeend',`<button class="button secondary" type="button" data-order-financial-correct="${order.id}" ${financialAllowed?'':'disabled'}>更正收款</button>`);
  }
...
function renderServiceChangeStoreOptions() {
  ...
  select.innerHTML = `<option value="">全部可管理门店</option>${(selectableStores || []).map(store => `<option value="${store.id}">${roomTransferEscape(store.name)}</option>`).join('')}`;
...
async function loadServiceChangeTechnicians() {
  ...
  const rows = await response.json();
  select.innerHTML += rows.map(item => `<option value="${item.id}">${roomTransferEscape(item.code)} · ${roomTransferEscape(item.name)}</option>`).join('');
}
...
async function loadServiceChangeHistoryCenter() {
  ...
  target.innerHTML = serviceChangeQueryRows.map(row => `<tr><td>${new Date(row.changedAt).toLocaleString('zh-CN')}</td><td>${roomTransferEscape(row.storeName)}</td><td><span class="service-change-type type-${String(row.eventType).toLowerCase()}">${serviceChangeTypeLabel[row.eventType] || row.eventType}</span></td><td><b>${roomTransferEscape(row.technicianName)}</b><small class="muted-cell">${roomTransferEscape(row.roomCode)} 房</small></td><td><b>${roomTransferEscape(row.serviceNameSnapshot)}</b><small class="muted-cell">${roomTransferEscape(row.orderNo || '未结算')}</small></td><td class="service-change-detail-cell">${roomTransferEscape(row.detail)}</td><td>${roomTransferEscape(row.reason || '—')}</td><td>${roomTransferEscape(row.actorName || '系统')}</td><td><button class="text-button" type="button" data-service-change-session="${row.serviceSessionId}">查看服务</button></td></tr>`).join('') || '<tr><td colspan="9" class="table-empty">所选条件没有服务变更记录</td></tr>';
}
...
function renderServiceSessions() {
  ...
  const durationAction = session.status === 'IN_SERVICE' && hasAdminPermission('SERVICE_DURATION_OVERRIDE')
    ? `<button type="button" class="text-button" data-duration-session="${session.id}">调整时长</button>` : '<span class="muted-cell">-</span>';
  return `<tr><td><b>${memberBusinessEscape(session.serviceNo||String(session.id).slice(0,8))}</b></td><td>${formatServiceDateTime(session.startedAt)}</td><td>${formatServiceDateTime(session.endedAt)}</td><td><b>${memberBusinessEscape(session.orderNo||'未结算')}</b><small class="muted-cell">${memberBusinessEscape(session.settlementNo||'—')}</small></td><td><b>${memberBusinessEscape(session.technicianName)}</b><small class="muted-cell">${memberBusinessEscape(session.roomCode)} 房</small></td><td><span class="record-type ${isCallClockType(session.clockType) ? 'consumption' : 'order'}">${clockType}</span><small class="service-item-detail">${memberBusinessEscape(session.serviceNameSnapshot+extension)}</small></td><td class="amount-cell">${money(receivable/100)}</td><td class="amount-cell">${paid==null?'—':money(paid/100)}</td><td>${memberBusinessEscape(session.paymentMethods||'未结算')}</td><td class="amount-cell commission-amount">${commission}</td><td><span class="service-status ${status[1]}">${status[0]}</span></td><td><div class="service-record-actions">${durationAction}<button type="button" class="text-button" data-change-history-session="${session.id}">变更记录</button></div></td></tr>`;
...
function renderOrderCommissionAdjustments(){
  ...
  document.querySelector('#commission-adjustments').innerHTML=rows.map(row=>`<tr><td>${formatServiceDateTime(row.settledAt)}</td><td><b>${memberBusinessEscape(row.adjustmentReferenceNo||row.orderNoSnapshot||'—')}</b><small class="muted-cell">${memberBusinessEscape(row.orderNoSnapshot||'—')}</small></td><td>${memberBusinessEscape(row.settlementNoSnapshot||'—')}</td><td>${memberBusinessEscape(row.technicianNameSnapshot||'—')}</td><td>${memberBusinessEscape(row.serviceNameSnapshot||'—')}</td><td><span class="record-type refund-state">${commissionAdjustmentTypeLabel[row.recordType]||'提成调整'}</span></td><td class="amount-cell refund-signed-amount">${signedMoneyCents(row.baseAmountCents)}</td><td class="amount-cell refund-signed-amount">${signedMoneyCents(row.commissionCents)}</td><td>${memberBusinessEscape(row.adjustmentReason||'系统调整')}</td></tr>`).join('')||'<tr><td colspan="9" class="table-empty">所选时间没有提成调整记录</td></tr>';
}
...
function renderMonthlyCommissionTierPolicy() {
  ...
  const scheduled = monthlyCommissionTierPolicy.scheduled ? `<small>已预约 ${monthlyCommissionTierPolicy.scheduled.effectiveBusinessDate} 生效的新策略</small>` : '';
  target.innerHTML = `<div><b>${policy.active ? '已启用' : '未启用'}</b><small>${policy.effectiveBusinessDate} 起生效</small></div><div class="monthly-tier-pills">${policy.tiers.map(tier => `<span><b>${tierEscape(tier.tierName)}</b> ${tier.minimumMonthlyClockCount}+ 钟 · ${(Number(tier.commissionMultiplierBp) / 100).toFixed(2)}%</span>`).join('')}</div>${scheduled}`;
}
...
function openClockDialog(tech = null, selectedRoom = null) {
  ...
  document.querySelector('#dispatch-tech-list').innerHTML = availableTechs.map(item => `<button class="dispatch-tech-choice" data-dispatch-tech="${item.id}" type="button"><span class="tech-avatar">${item.initials}</span><span><b>${item.name}</b><small>轮钟 ${String(item.queue).padStart(2, '0')}</small></span><em>${item.state === 'available' ? '可派' : '服务中'}</em></button>`).join('');
...
function financeReportStoreOptions(summary){const select=document.querySelector('#finance-report-store');const current=select.value;const stores=summary.stores||[];select.innerHTML='<option value="">全部门店</option>'+stores.map(item=>`<option value="${item.storeId}">${financeEscape(item.storeName)}</option>`).join('');if(stores.some(item=>item.storeId===current))select.value=current;}
function financeReportMetric(label,value){return `<div class="finance-report-metric"><span>${label}</span><strong>${value}</strong></div>`;}
function renderFinanceReport(summary){financeReport=summary;financeReportStoreOptions(summary);const totals=summary.totals||{};document.querySelector('#finance-report-metrics').innerHTML=[financeReportMetric('报销单数',totals.claimCount||0),financeReportMetric('申请总额',financeReportMoney(totals.totalAmountCents)),financeReportMetric('待审核',`${totals.pendingCount||0} 单`),financeReportMetric('待付款',financeReportMoney(totals.approvedAmountCents)),financeReportMetric('已付款',financeReportMoney(totals.paidAmountCents))].join('');document.querySelector('#finance-report-stores').innerHTML=(summary.stores||[]).map(item=>`<tr><td>${financeEscape(item.storeName)}</td><td>${item.claimCount||0}</td><td class="amount-cell">${financeReportMoney(item.amountCents)}</td><td class="amount-cell">${financeReportMoney(item.paidAmountCents)}</td></tr>`).join('')||'<tr><td colspan="4" class="table-empty">暂无数据</td></tr>';document.querySelector('#finance-report-categories').innerHTML=(summary.categories||[]).map(item=>`<tr><td>${financeEscape(item.categoryName)}</td><td>${item.claimCount||0}</td><td class="amount-cell">${financeReportMoney(item.amountCents)}</td><td class="amount-cell">${financeReportMoney(item.paidAmountCents)}</td></tr>`).join('')||'<tr><td colspan="4" class="table-empty">暂无数据</td></tr>';const statusLabel={SUBMITTED:'待审核',APPROVED:'待付款',PAID:'已付款',RETURNED:'已退回',REJECTED:'已驳回'};document.querySelector('#finance-report-statuses').innerHTML=(summary.statuses||[]).map(item=>`<tr><td>${statusLabel[item.status]||financeEscape(item.status)}</td><td>${item.claimCount||0}</td><td class="amount-cell">${financeReportMoney(item.amountCents)}</td></tr>`).join('')||'<tr><td colspan="3" class="table-empty">暂无数据</td></tr>';}
...
function renderMemberConsumeOrders(){const list=document.querySelector('#member-consume-orders');if(!list)return;list.innerHTML=state.pendingServiceSessions.map(session=>{const selected=memberBusinessConsumeSelection.has(session.id);const extension=session.extensionSummary?` · 加钟：${session.extensionSummary}`:'';return `<button type="button" class="member-consume-order${selected?' selected':''}" data-member-consume-order="${session.id}"><i>${selected?'✓':''}</i><span><b>${memberBusinessEscape(session.serviceNameSnapshot)}</b><small>${clockTypeLabels[session.clockType]||session.clockType} · ${memberBusinessEscape(session.technicianName)} · ${memberBusinessEscape(session.roomCode)} 房 · ${session.plannedDurationMinutes} 分钟${memberBusinessEscape(extension)}</small></span><strong>${money(Number(session.servicePriceCents||0)/100)}</strong></button>`;}).join('')||'<p>暂无待结算服务</p>';const total=[...memberBusinessConsumeSelection].map(id=>state.pendingServiceSessions.find(item=>item.id===id)).filter(Boolean).reduce((sum,item)=>sum+Number(item.servicePriceCents||0),0);document.querySelector('#member-consume-count').textContent=memberBusinessConsumeSelection.size;document.querySelector('#member-consume-total').textContent=money(total/100);document.querySelector('#member-consume-after').textContent=money(((memberBusinessConsumeMember?.balanceCents||0)-total)/100);}
...
async function loadMemberOpenCardOptions(){
  ...
  const technicianOptions=`<option value="">未关联技师</option>${technicians.map(item=>`<option value="${item.id}">${memberBusinessEscape(item.code)} · ${memberBusinessEscape(item.name)}</option>`).join('')}`;
  const employeeOptions=`<option value="">未关联员工</option>${employees.map(item=>`<option value="${item.employeeId}">${memberBusinessEscape(item.employeeNo||'')} · ${memberBusinessEscape(item.fullName)}${item.positionName?` · ${memberBusinessEscape(item.positionName)}`:''}</option>`).join('')}`;
...
async function openSingleRoomSettlementSelection() {
  ...
  const select = document.querySelector('#single-room-settlement-room');
  select.innerHTML = rooms.map(room => `<option value="${room.apiId}">${memberBusinessEscape(room.id)} 房 · ${memberBusinessEscape(room.detail || '待付款服务')}</option>`).join('');
...
async function loadTechnicianAccounts() { const response = await fetch('http://localhost:8080/api/v1/admin/technician-accounts',{headers:storeContextHeaders()}); if (!response.ok) throw new Error(); technicianAccounts = await response.json(); document.querySelector('#account-tech').innerHTML = technicianAccounts.filter(item => !item.bindingId).map(item => `<option value="${item.technicianId}">${item.technicianCode} · ${item.technicianName}</option>`).join('') || '<option value="">所有启用技师均已绑定</option>'; document.querySelector('#tech-account-records').innerHTML = technicianAccounts.map(item => `<article class="technician-account-row"><div class="technician-account-identity"><b>${item.technicianName}</b><small>技师编号：${item.technicianCode}</small><small>登录账号：${item.loginName || '未创建账号'}</small></div><div class="technician-account-status"><em class="record-type ${item.bindingId && item.active ? 'order' : 'consumption'}">${item.bindingId ? (item.active ? '已启用' : '已停用') : '未绑定'}</em><small>密码：${item.bindingId ? '已加密保存，不可查看' : '—'}</small></div><div class="technician-account-actions">${item.bindingId && item.active ? `<button class="record-delete edit-technician" type="button" data-account-reset="${item.bindingId}">重置密码</button><button class="record-delete" type="button" data-account-toggle="${item.bindingId}" data-active="true">停用</button>` : item.bindingId ? `<button class="record-delete edit-technician" type="button" data-account-toggle="${item.bindingId}" data-active="false">重新启用</button>` : '<small>请先创建登录账号</small>'}</div></article>`).join('') || '<p class="empty-state">暂无技师账号记录</p>'; }
...
document.querySelector('#manage-beds').addEventListener('click', async () => {
  ...
  document.querySelector('#bed-room').innerHTML = rooms.filter(room => room.active).map(room => `<option value="${room.id}">${room.code} · ${room.name}</option>`).join('');
  document.querySelector('#bed-results').innerHTML = beds.map(bed => `<div class="member-result"><span class="member-avatar">床</span><span><b>${bed.code}</b><small>${bed.name} · 排序 ${bed.sortOrder}</small></span><em>${bed.active ? '启用' : '已删除'}</em>${bed.active ? `<button class="record-delete" data-bed-edit="${bed.id}">编辑</button><button class="record-delete" data-bed-active="${bed.id}" data-active="true">删除</button>` : '<span class="muted-cell">—</span>'}</div>`).join('') || '<p class="empty-state">暂无床位</p>';
...
document.querySelector('#frontdesk-view').addEventListener('click', async event => {
  ...
  if (refresh) { await loadRoomTransferRequests().catch(() => {}); return; }
...
function financeCategoryOptions(selectedId = '') {
  return financeExpenseCategories.filter(item => item.active && item.code !== 'PENDING_FINANCE_CLASSIFICATION').map(item => `<option value="${item.id}"${item.id === selectedId ? ' selected' : ''}>${financeEscape(item.parentName ? `${item.parentName} / ${item.name}` : item.name)}</option>`).join('');
}
...
function openFinanceCategoryDialog(item = null) {
  ...
  parent.innerHTML = '<option value="">作为一级分类</option>' + financeExpenseCategories.filter(row => row.active && row.categoryLevel === 1 && row.code !== 'PENDING_FINANCE_CLASSIFICATION' && row.id !== item?.id).map(row => `<option value="${row.id}">${financeEscape(row.name)}</option>`).join('');
